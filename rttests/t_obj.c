...
static PyDosObj far * PYDOS_API builtin_pir_probe(
    PyDosObj far *self, PyDosObj far *a1, PyDosObj far *a2,
    PyDosObj far *a3, PyDosObj far *a4, PyDosObj far *a5,
    PyDosObj far *a6, PyDosObj far *a7)
{
    (void)self; (void)a1; (void)a2; (void)a3;
    (void)a4; (void)a5; (void)a6; (void)a7;
    return pydos_obj_new_str((const char far *)"python-backed", 13);
}

static PyDosObj far * PYDOS_API builtin_pir_probe_replacement(
    PyDosObj far *self, PyDosObj far *a1, PyDosObj far *a2,
    PyDosObj far *a3, PyDosObj far *a4, PyDosObj far *a5,
    PyDosObj far *a6, PyDosObj far *a7)
{
    (void)self; (void)a1; (void)a2; (void)a3;
    (void)a4; (void)a5; (void)a6; (void)a7;
    return pydos_obj_new_str((const char far *)"replaced", 8);
}
...
{
    PyDosObj far *obj = pydos_obj_alloc_type(PYDT_INT);
    ASSERT_NOT_NULL(obj);
    pydos_obj_free(obj);
}
...
TEST(get_iter_range)
{
    PyDosObj far *rng;
    PyDosObj far *iter;
    rng = pydos_obj_alloc_type(PYDT_RANGE);
    ASSERT_NOT_NULL(rng);
    rng->refcount = 1;
    rng->flags = 0;
    rng->v.range.start = 0;
...
TEST(call_method_builtin_pir_vtable)
{
    PyDosObj far *self;
    PyDosObj far *argv[1];
    PyDosObj far *ret;
    ...
    self = pydos_obj_new_str((const char far *)"value", 5);
    argv[0] = self;
    pydos_builtin_vtable_add_method(
        PYDT_STR,
        (const char far *)"_pir_probe",
        (void (far *)(void))builtin_pir_probe);

    ret = pydos_obj_call_method((const char far *)"_pir_probe", 1, argv);
    ASSERT_NOT_NULL(ret);
    ASSERT_EQ(ret->type, PYDT_STR);
    ASSERT_STR_EQ(ret->v.str.data, "python-backed");
    PYDOS_DECREF(ret);

    method_count = pydos_builtin_vtables[PYDT_STR]->method_count;
    pydos_builtin_vtable_add_method(
        PYDT_STR,
        (const char far *)"_pir_probe",
        (void (far *)(void))builtin_pir_probe_replacement);
    ASSERT_EQ(pydos_builtin_vtables[PYDT_STR]->method_count, method_count);

    ret = pydos_obj_call_method((const char far *)"_pir_probe", 1, argv);
    ASSERT_NOT_NULL(ret);
    ASSERT_STR_EQ(ret->v.str.data, "replaced");
...
{
    PyDosObj far *t = pydos_obj_alloc_type(PYDT_TUPLE);
    t->refcount = 1;
    t->v.tuple.len = 3;
...
TEST(del_attr_basic)
{
    ...
    PyDosObj far *obj = pydos_obj_alloc_type(PYDT_INSTANCE);
    ...
    got = pydos_obj_get_attr(obj, "x");
    /* Python attribute lookup raises AttributeError after deletion. */
    ASSERT_NULL(got);
    ASSERT_TRUE(pydos_exc_matches(pydos_exc_current(),
                                  PYDOS_EXC_ATTRIBUTE_ERROR));
    pydos_exc_clear();
...
void run_obj_tests(void)
{
    ...
    RUN(call_method_builtin_pir_vtable);
    RUN(obj_add_ints);
    RUN(obj_add_strs);
    RUN(obj_sub_ints);
    RUN(obj_mul_ints);
