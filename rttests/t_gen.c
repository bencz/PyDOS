 ...
 *
 * check_throw reports the pending throw to the generated state machine.
 * This hand-written resume function has no landing pad, so it propagates
 * the exception to the caller by returning NULL.
 *
 ...
 * and resume returns NULL. This hand-written state machine has no
 * GeneratorExit landing pad, so the "ignored GeneratorExit" path is
 * unreachable.
 *
 * pc=0: yield 1, set pc=1
...
/* ------------------------------------------------------------------ */
/* gen_throw_handled: hand-written resume has no exception landing pad */
/* ------------------------------------------------------------------ */
...
TEST(gen_close_active)
{
     ...
     * close marks gen exhausted (pc=-1). */
    pydos_gen_close(g);
    ASSERT_EQ(g->v.gen.pc, -1);
...
/* non-local C control transfer needed.                                */
/* ------------------------------------------------------------------ */
...
/* ------------------------------------------------------------------ */
/* gen_check_throw_signals: flag set → returns 1                       */
/* ------------------------------------------------------------------ */
