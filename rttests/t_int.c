...
TEST(bit_methods)
{
    ...
    value = pydos_obj_new_int(-181L);
    length = pydos_int_bit_length(value);
    count = pydos_int_bit_count(value);
    ASSERT_NOT_NULL(length);
    ASSERT_NOT_NULL(count);
    ASSERT_EQ(length->v.int_val, 8L);
    ASSERT_EQ(count->v.int_val, 5L);
    PYDOS_DECREF(count);
    PYDOS_DECREF(length);
    PYDOS_DECREF(value);
}

TEST(float_numeric_semantics)
{
    PyDosObj far *a;
    PyDosObj far *b;
    PyDosObj far *result;
    ...
    a = pydos_obj_new_float(-7.5);
    b = pydos_obj_new_float(2.0);
    result = pydos_int_div(a, b);
    ASSERT_EQ(result->type, PYDT_FLOAT);
    ASSERT_TRUE(result->v.float_val == -4.0);
    PYDOS_DECREF(result);

    result = pydos_int_mod(a, b);
    ASSERT_EQ(result->type, PYDT_FLOAT);
    ASSERT_TRUE(result->v.float_val == 0.5);
    PYDOS_DECREF(result);

    PYDOS_DECREF(a);
    PYDOS_DECREF(b);

    a = pydos_obj_new_int(2L);
    exp = pydos_obj_new_int(-2L);
    result = pydos_int_pow(a, exp);
    ASSERT_EQ(result->type, PYDT_FLOAT);
    ASSERT_TRUE(result->v.float_val == 0.25);
    PYDOS_DECREF(result);
    PYDOS_DECREF(exp);
    PYDOS_DECREF(a);
}

TEST(numeric_methods_and_attributes)
{
    PyDosObj far *value;
    PyDosObj far *result;

    value = pydos_obj_new_float(4.0);
    result = pydos_float_is_integer(value);
    ASSERT_TRUE(pydos_obj_is_truthy(result));
    PYDOS_DECREF(result);
    result = pydos_obj_get_attr(value, (const char far *)"real");
    ASSERT_TRUE(result == value);
    PYDOS_DECREF(result);
    result = pydos_obj_get_attr(value, (const char far *)"imag");
    ASSERT_EQ(result->type, PYDT_FLOAT);
    ASSERT_TRUE(result->v.float_val == 0.0);
    PYDOS_DECREF(result);
    PYDOS_DECREF(value);

    value = pydos_obj_new_int(5L);
    result = pydos_obj_get_attr(value, (const char far *)"numerator");
    ASSERT_TRUE(result == value);
    PYDOS_DECREF(result);
    result = pydos_obj_get_attr(value, (const char far *)"denominator");
...
void run_int_tests(void)
{
    ...
    RUN(from_str);
    RUN(bit_methods);
    RUN(float_numeric_semantics);
    RUN(numeric_methods_and_attributes);
}
