...
TEST(stats_bytes)
{
    ...
    ASSERT_EQ(after_alloc, before + 128);

    pydos_far_free(p);
    after_free = pydos_mem_current_bytes();
    ASSERT_EQ(after_free, before);
}

TEST(stats_kinds)
{
    unsigned long object_before;
    unsigned long buffer_before;
    void far *object_block;
    void far *buffer_block;

    object_before = pydos_mem_kind_current_bytes(PYDOS_MEM_OBJECT);
    buffer_before = pydos_mem_kind_current_bytes(PYDOS_MEM_BUFFER);
    object_block = pydos_mem_alloc(PYDOS_MEM_OBJECT, 48UL);
    buffer_block = pydos_mem_alloc(PYDOS_MEM_BUFFER, 96UL);
    ASSERT_NOT_NULL(object_block);
    ASSERT_NOT_NULL(buffer_block);
    ASSERT_EQ(pydos_mem_kind_current_bytes(PYDOS_MEM_OBJECT),
              object_before + 48UL);
    ASSERT_EQ(pydos_mem_kind_current_bytes(PYDOS_MEM_BUFFER),
              buffer_before + 96UL);
    ASSERT_TRUE(pydos_mem_kind_peak_bytes(PYDOS_MEM_OBJECT) >=
                object_before + 48UL);
    ASSERT_TRUE(pydos_mem_peak_bytes() >= pydos_mem_current_bytes());
    pydos_far_free(buffer_block);
    pydos_far_free(object_block);
    ASSERT_EQ(pydos_mem_kind_current_bytes(PYDOS_MEM_OBJECT), object_before);
    ASSERT_EQ(pydos_mem_kind_current_bytes(PYDOS_MEM_BUFFER), buffer_before);
}

TEST(stats_realloc_exact)
{
    unsigned long before;
    void far *p;

    before = pydos_mem_current_bytes();
    p = pydos_mem_alloc(PYDOS_MEM_METADATA, 20UL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pydos_mem_current_bytes(), before + 20UL);
    p = pydos_mem_realloc(p, 75UL);
    ASSERT_NOT_NULL(p);
    ASSERT_EQ(pydos_mem_current_bytes(), before + 75UL);
    pydos_far_free(p);
    ASSERT_EQ(pydos_mem_current_bytes(), before);
}
