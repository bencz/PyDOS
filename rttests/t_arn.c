...
/* ------------------------------------------------------------------ */
/* arn_track_preserves_flags: ledger tracking does not mutate objects   */
/* ------------------------------------------------------------------ */

TEST(arn_track_preserves_flags)
{
    PyDosObj far *obj;
...
TEST(arn_nested_scopes)
{
    ...
    ASSERT_EQ(outer_obj->type, PYDT_INT);
    ASSERT_EQ(outer_obj->v.int_val, 1000L);
    ASSERT_EQ(outer_obj->refcount, 1);
    pydos_arena_scope_exit();
    /* outer_obj freed now */
}
...
/* ------------------------------------------------------------------ */
/* arn_immortal_skipped: tracking does not mutate immortal objects      */
/* ------------------------------------------------------------------ */
...
TEST(arn_immortal_skipped)
{
    ...
    ASSERT_TRUE((none_obj->flags & OBJ_FLAG_IMMORTAL) != 0);
    pydos_arena_scope_exit();
