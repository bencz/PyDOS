...
int PYDOS_API pydos_dos_file_close(int handle)
{
    union REGS inregs, outregs;
...
PyDosObj far * PYDOS_API pydos_io_file_open(int argc,
                                             PyDosObj far * far *argv)
{
    ...
    if (argc > 1 && argv[1] != (PyDosObj far *)0 &&
        (PyDosType)argv[1]->type == PYDT_STR && argv[1]->v.str.len > 0) {
        if (argv[1]->v.str.data[0] == 'w') mode = 1;
        else if (argv[1]->v.str.data[0] == 'r' &&
                 argv[1]->v.str.len > 1 && argv[1]->v.str.data[1] == '+')
            mode = 2;
    }
    handle = pydos_dos_file_open(argv[0]->v.str.data, mode);
    return pydos_obj_new_int((long)handle);
}

PyDosObj far * PYDOS_API pydos_io_file_read(int argc,
                                             PyDosObj far * far *argv)
{
    int handle;
    unsigned int count;
    ...
    if (argc < 2) {
        pydos_exc_raise(PYDOS_EXC_TYPE_ERROR,
                        (const char far *)"file read requires handle and count");
        return (PyDosObj far *)0;
    }
    handle = (int)(argv[0] != (PyDosObj far *)0
                   ? argv[0]->v.int_val : -1L);
    count = (unsigned int)(argv[1] != (PyDosObj far *)0
                           ? argv[1]->v.int_val : 0L);
    if (count == 0) return pydos_obj_new_str((const char far *)"", 0);
    if (count > 32767U) count = 32767U;
    ...
    if (buffer == (char far *)0) {
        pydos_exc_raise(PYDOS_EXC_MEMORY_ERROR,
                        (const char far *)"cannot allocate file read buffer");
        return (PyDosObj far *)0;
    }
    read_count = pydos_dos_file_read(handle, buffer, count);
    if (read_count < 0) {
        pydos_far_free(buffer);
        pydos_exc_raise(PYDOS_EXC_OS_ERROR,
                        (const char far *)"DOS file read failed");
        return (PyDosObj far *)0;
    }
    result = pydos_obj_new_str(buffer, (unsigned int)read_count);
    pydos_far_free(buffer);
    return result;
}

PyDosObj far * PYDOS_API pydos_io_file_write(int argc,
                                              PyDosObj far * far *argv)
{
    int handle;
    int written;
    if (argc < 2 || argv[1] == (PyDosObj far *)0 ||
        ...
        pydos_exc_raise(PYDOS_EXC_TYPE_ERROR,
                        (const char far *)"file write requires text data");
        return (PyDosObj far *)0;
    }
    handle = (int)(argv[0] != (PyDosObj far *)0
                   ? argv[0]->v.int_val : -1L);
    written = pydos_dos_file_write(handle, argv[1]->v.str.data,
                                   argv[1]->v.str.len);
    if (written < 0) {
        pydos_exc_raise(PYDOS_EXC_OS_ERROR,
                        (const char far *)"DOS file write failed");
        return (PyDosObj far *)0;
    }
    return pydos_obj_new_int((long)written);
}
                                              ...
                                              PyDosObj far * far *argv)
{
    if (argc < 1 || argv[0] == (PyDosObj far *)0) {
        pydos_exc_raise(PYDOS_EXC_TYPE_ERROR,
                        (const char far *)"file close requires a handle");
        return (PyDosObj far *)0;
    }
    if (pydos_dos_file_close((int)argv[0]->v.int_val) < 0) {
        pydos_exc_raise(PYDOS_EXC_OS_ERROR,
                        (const char far *)"DOS file close failed");
        return (PyDosObj far *)0;
    }
