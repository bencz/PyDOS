 ...
 *  8. VTable mechanism
 *  9. Dictionary operations (creates sentinel)
 * 10. List operations
 * 11. Generator helpers
 * 12. Built-in functions
 * 13. Create global namespace
 */
void PYDOS_API pydos_rt_init(void)
{
    ...
    /* 7. Exception handling.  It precedes metadata allocation so an
     * allocation failure can always produce the static MemoryError. */
    pydos_exc_init();

    /* 8. VTable mechanism (creates builtin type vtables) */
    pydos_vtable_init();

    /* 9. Dictionary operations (creates deleted sentinel) */
    pydos_dict_init();
    ...
    /* 11b. Cell objects (closures) */
    pydos_cell_init();

    /* 11c. Coroutine / async helpers */
    pydos_cor_init();

    /* 11d. Frozenset operations */
    pydos_frozenset_init();

    /* 12. Built-in functions */
    pydos_builtins_init();

    /* 12b. Low-overhead sys.monitoring registry */
    pydos_monitoring_init();

    /* 13. Create global namespace as empty dict */
...
void PYDOS_API pydos_rt_shutdown(void)
{
    ...
    /* Release compiler-owned temporary references before deciding which
     * tracked components are unreachable. */
    ...
    /* Release every subsystem-owned reference while objects and GC are
     * still fully operational. */
    pydos_monitoring_shutdown();
    ...
    /* Collect cycles exposed by subsystem shutdown, then dispose of any
     * defensive leftovers before flushing the scalar-object free list. */
    pydos_gc_collect();
    pydos_gc_shutdown();
    pydos_obj_shutdown();
    pydos_mem_shutdown();
