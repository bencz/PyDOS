...
/* Raise StopIteration while preserving a generator return value. */
...
/* Return an owned reference to the pending exception. */
PyDosObj far * PYDOS_API pydos_exc_fetch(void);

/* Add the current generated frame to the pending exception traceback. */
...
/* Return an owned reference to an exception's traceback, or None. */
PyDosObj far * PYDOS_API pydos_exc_get_traceback(PyDosObj far *exc);

/* Non-zero when an exception is pending. */
int PYDOS_API pydos_exc_pending(void);

/* Clear current exception */
void PYDOS_API pydos_exc_clear(void);

/* Report the pending exception as unhandled and terminate. */
