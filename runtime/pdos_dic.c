...
#define DICT_MIN_SIZE 4
#define DICT_LAZY_SIZE 4
...
static void dict_resize(PyDosObj far *dict, unsigned int new_size)
{
    ...
    new_entries = (PyDosDictEntry far *)pydos_mem_alloc(
...
PyDosObj far * PYDOS_API pydos_dict_new(unsigned int initial_size)
{
    ...
    lazy = initial_size == 0;
    if (!lazy && initial_size < DICT_MIN_SIZE) {
        initial_size = DICT_MIN_SIZE;
    }

    /* Round up to power of 2.  Zero is the intentional lazy sentinel. */
    if (!lazy) {
        unsigned int s;
        s = 1;
    ...
    dict = pydos_obj_alloc_type(PYDT_DICT);
    ...
    /* Class namespaces are commonly empty or contain only compiled methods,
     * which already live in their vtable.  A zero-size dictionary keeps its
     * object identity for reflection but defers the entry array until the
     * first materialized attribute is stored. */
    if (lazy) {
        dict->v.dict.entries = (PyDosDictEntry far *)0;
        return dict;
    }

    alloc_size = (unsigned long)initial_size * (unsigned long)sizeof(PyDosDictEntry);
    dict->v.dict.entries = (PyDosDictEntry far *)pydos_mem_alloc(
        PYDOS_MEM_METADATA, alloc_size);
                    ...
                    PyDosObj far *value)
{
    ...
    if (dict->v.dict.size == 0) {
        dict_resize(dict, DICT_LAZY_SIZE);
        if (dict->v.dict.size == 0) {
            pydos_exc_raise(PYDOS_EXC_MEMORY_ERROR,
                            (const char far *)"cannot allocate dictionary entries");
            return;
        }
    }
...
PyDosObj far * PYDOS_API pydos_dict_get_op(PyDosObj far *dict,
                                            PyDosObj far *key)
{
    PyDosObj far *result;
    if (dict == (PyDosObj far *)0 || dict->type != PYDT_DICT) {
        pydos_exc_raise(PYDOS_EXC_TYPE_ERROR,
                        (const char far *)"subscription requires a dict");
        return (PyDosObj far *)0;
    }
    result = pydos_dict_get(dict, key);
...
PyDosObj far * PYDOS_API pydos_dict_clear_m(PyDosObj far *dict)
{
    if (dict == (PyDosObj far *)0 || dict->type != PYDT_DICT) {
        pydos_exc_raise(PYDOS_EXC_TYPE_ERROR,
                        (const char far *)"clear requires a dict");
        return (PyDosObj far *)0;
    }
    pydos_dict_clear(dict);
...
PyDosObj far * PYDOS_API pydos_dict_items(PyDosObj far *dict)
{
    ...
    /* Iterate in insertion order */
    {
        ...
        while (emitted < dict->v.dict.used) {
            ...
            tuple = pydos_obj_alloc_type(PYDT_TUPLE);
            if (tuple != (PyDosObj far *)0) {
                tuple->refcount = 1;
                tuple->v.tuple.len = 2;
                tuple->v.tuple.items = (PyDosObj far * far *)pydos_mem_alloc(
                    PYDOS_MEM_METADATA,
...
/* Python-callable set mutation methods.  The lower-level mutation helpers
 * keep their compact C signatures for the VM and runtime, while these entry
 * points implement the object-returning Python call ABI. */
PyDosObj far * PYDOS_API pydos_set_add_m(PyDosObj far *self,
                                          PyDosObj far *item)
{
    pydos_set_add(self, item);
    if (pydos_exc_pending()) return (PyDosObj far *)0;
    return pydos_obj_new_none();
}

PyDosObj far * PYDOS_API pydos_set_remove_m(PyDosObj far *self,
                                             PyDosObj far *item)
{
    pydos_set_remove(self, item);
    if (pydos_exc_pending()) return (PyDosObj far *)0;
    return pydos_obj_new_none();
}

PyDosObj far * PYDOS_API pydos_set_discard_m(PyDosObj far *self,
                                              PyDosObj far *item)
{
    pydos_set_discard(self, item);
    if (pydos_exc_pending()) return (PyDosObj far *)0;
    return pydos_obj_new_none();
}

PyDosObj far * PYDOS_API pydos_set_clear_m(PyDosObj far *self)
{
    pydos_set_clear(self);
    if (pydos_exc_pending()) return (PyDosObj far *)0;
    return pydos_obj_new_none();
}

PyDosObj far * PYDOS_API pydos_set_empty(PyDosObj far *self)
{
    PyDosObj far *result;
    (void)self;
    result = pydos_dict_new(8);
    if (result != (PyDosObj far *)0) result->type = PYDT_SET;
    return result;
}
...
void PYDOS_API pydos_dict_init(void)
{
    ...
    dict_deleted_sentinel = pydos_obj_alloc_type(PYDT_NONE);
