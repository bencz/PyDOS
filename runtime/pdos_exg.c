    ...
    unsigned int count)
{
    ...
    obj = pydos_obj_alloc_type(PYDT_EXC_GROUP);
    if (obj == (PyDosObj far *)0) return (PyDosObj far *)0;

    arr = (PyDosObj far * far *)pydos_mem_alloc(PYDOS_MEM_METADATA,
    ...
    int argc, PyDosObj far * far *argv)
{
    ...
    matched_arr = (PyDosObj far * far *)pydos_mem_alloc(PYDOS_MEM_METADATA,
        (unsigned long)count * (unsigned long)sizeof(PyDosObj far *));
    unmatched_arr = (PyDosObj far * far *)pydos_mem_alloc(PYDOS_MEM_METADATA,
        (unsigned long)count * (unsigned long)sizeof(PyDosObj far *));
