                                              ...
                                              int count)
{
    ...
    fs = pydos_obj_alloc_type(PYDT_FROZENSET);
    ...
    buf = (PyDosObj far * far *)pydos_mem_alloc(PYDOS_MEM_METADATA,
    ...
    if (deduped < count) {
        ...
        newbuf = (PyDosObj far * far *)pydos_mem_alloc(PYDOS_MEM_METADATA,
...
{
    (void)self;
    if (items == (PyDosObj far *)0 ||
        (PyDosType)items->type != PYDT_LIST) {
        return pydos_frozenset_new((PyDosObj far * far *)0, 0);
    }
    return pydos_frozenset_new(items->v.list.items,
                               (int)items->v.list.len);
}
