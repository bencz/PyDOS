...
PIRLowerer::PIRLowerer()
    : ir_mod(0), pir_mod(0), current_func(0),
      error_count(0), current_source_line(0),
      block_labels(0), block_labels_cap(0),
...
IRInstr *PIRLowerer::emit(IROp op, int dest, int src1, int src2, int extra)
{
    ...
    instr->src2 = src2;
    instr->extra = extra;
    instr->line = current_source_line;
    instr->guard_target = 0;
    instr->type_hint = 0;
    instr->next = 0;
...
void PIRLowerer::lower_inst(PIRInst *inst, PIRFunction * /*pir_func*/)
{
    ...
    case PIR_CALL:
        {
            /* Explicit callable operands are deliberately indirect.  src2
             * marks them so codegen cannot fold a decorated global back to
             * its original function label. */
            if (inst->num_operands > 0 &&
                pir_value_valid(inst->operands[0])) {
                emit(IR_CALL, dest, val_temp(inst->operands[0]), 1,
                     inst->int_val);
                break;
            }
    ...
    case PIR_GUARDED_CALL_METHOD:
        ci = add_const_str(
            ...
            inst->str_val ? (int)strlen(inst->str_val) : 0);
        {
            IRInstr *guarded = emit(IR_GUARDED_CALL_METHOD, dest,
                                    val_temp(inst->operands[0]),
                                    ci, inst->int_val);
            guarded->guard_target = lower_str_dup(
                inst->guard_target ? inst->guard_target : "");
        }
        break;
    ...
    case PIR_RETURN:
        emit_phi_stores_for_block(current_pir_block_id);
        emit(IR_RETURN, -1, val_temp(inst->operands[0]), -1,
             inst->int_val);
        break;
    ...
    case PIR_MAKE_FUNCTION:
            ...
            inst->str_val ? (int)strlen(inst->str_val) : 0);
        {
            int display_ci = ci;
            if (inst->int_val >= 0 &&
                inst->int_val < pir_mod->string_constants.size()) {
                const char *display =
                    pir_mod->string_constants[(int)inst->int_val];
                if (display)
                    display_ci = add_const_str(display,
                                               (int)strlen(display));
            }
    ...
    case PIR_RAISE:
        emit(IR_RAISE, -1, val_temp(inst->operands[0]), -1,
             block_label(inst->handler_block));
        break;

    case PIR_RERAISE:
        emit(IR_RERAISE, -1, -1, -1,
             block_label(inst->handler_block));
        break;
    ...
    case PIR_CHECK_EXCEPTION:
        emit(IR_CHECK_EXCEPTION, -1,
             inst->num_operands > 0 ? val_temp(inst->operands[0]) : -1, -1,
             block_label(inst->handler_block));
        break;

    case PIR_CLEAR_EXCEPTION:
        emit(IR_CLEAR_EXCEPTION, -1, -1, -1, 0);
        break;

    case PIR_PANIC_EXCEPTION:
        emit(IR_PANIC_EXCEPTION, -1, -1, -1, 0);
        break;
    ...
    case PIR_FOR_ITER:
        /* extra is normal exhaustion; src2 is exceptional propagation. */
        emit(IR_FOR_ITER, dest, val_temp(inst->operands[0]),
             block_label(inst->false_block),
             block_label(inst->handler_block));
        break;
    ...
    case PIR_SCOPE_TRACK:
        emit(IR_SCOPE_TRACK, -1, val_temp(inst->operands[0]), -1,
             inst->int_val);
        break;
    case PIR_SCOPE_EXIT:
    ...
    /* Copy vtable info from PIR module to IR module */
    {
        ...
        for (vi = 0; vi < pm->num_vtables && vi < 128; vi++) {
            ...
            for (mi = 0; mi < src->num_methods && mi < 64; mi++) {
                ...
                dst->methods[mi].arg_count = src->methods[mi].arg_count;
            }
        }
    }
