...
/* A join is proven only when every incoming value has the same type.
 * Unknown is the top of this lattice, not an identity value. */
    ...
    while (changed && iteration < 10) {
        ...
        for (bi = 0; bi < dom->num_blocks; bi++) {
            ...
            for (inst = block->first; inst; inst = inst->next) {
                ...
                /* Later iterations may discover a conflicting predecessor,
                 * so an earlier proof must be allowed to regress. */
                if (new_type != vt[rid].proven_type) {
                    vt[rid].proven_type = new_type;
                    changed = 1;
                }
            }
        }
