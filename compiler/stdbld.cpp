 ...
 *   Pass 2: Compile pyfncs.py for module-level builtins and the type files
 *           themselves for Python-backed class methods.
...
/* Join a multiline Python function signature into the current buffer. */
static int complete_def_line(FILE *f, char *line, int capacity)
{
    int depth = 0;
    int i;
    for (i = 0; line[i]; i++) {
    ...
    while (depth > 0) {
        ...
        int used;
        int needed;
        if (!fgets(continuation, sizeof(continuation), f)) return 0;
        strip_trailing(continuation);
        trimmed = skip_ws(continuation);
        used = (int)strlen(line);
        needed = (int)strlen(trimmed);
        if (used + needed + 2 >= capacity) return 0;
        line[used++] = ' ';
        strcpy(line + used, trimmed);
        for (i = 0; trimmed[i]; i++) {
            if (trimmed[i] == '(') depth++;
...
static int ret_type_from_str(const char *s)
{
    ...
    if (strcmp(s, "bytes") == 0) return TK_BYTES;
...
static int type_kind_from_name(const char *s)
{
    static struct { const char *name; int kind; } map[] = {
        ...
        {"set", TK_SET}, {"int", TK_INT}, {"float", TK_FLOAT},
        {"tuple", TK_TUPLE},
        {"frozenset", TK_FROZENSET}, {"complex", TK_COMPLEX},
        {"bytearray", TK_BYTEARRAY}, {"bytes", TK_BYTES},
        {"range", TK_RANGE}, {0, 0}
    };
    int i;
...
static int parse_functions_file(const char *path)
{
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(trimmed, "def ", 4) == 0) {
            if (!complete_def_line(f, line, sizeof(line))) {
                fclose(f);
                return -1;
            }
            trimmed = skip_ws(line);
        ...
        if (strncmp(trimmed, "def ", 4) == 0) {
            ...
            if (pending_decorator) {
                ...
                /* Python-backed */
                /* The C symbol slot is unused for PIR functions.  Preserve a
                 * compact source signature there so callers compiled from a
                 * separate module can bind keywords and fill defaults. */
                if ((int)strlen(params) < STDLIB_ASM_LEN) {
                    strncpy(pf->asm_name, params, STDLIB_ASM_LEN - 1);
                } else {
                    pf->asm_name[0] = '\0';
                }
                pf->exc_code = -1;
    ...
    while (fgets(line, sizeof(line), f)) {
        ...
        if (strncmp(trimmed, "def ", 4) == 0 && pt) {
            ...
            if (pending_decorator) {
                ...
                /* Store the signature excluding self so calls compiled in a
                 * separate module can bind keywords and real defaults. */
                const char *method_params = strchr(params, ',');
                if (method_params) {
                    method_params++;
                    while (*method_params == ' ' || *method_params == '\t')
                        method_params++;
                } else {
                    method_params = "";
                }
                if ((int)strlen(method_params) < STDLIB_ASM_LEN) {
                    strncpy(pm->asm_name, method_params,
                            STDLIB_ASM_LEN - 1);
                } else {
                    pm->asm_name[0] = '\0';
                }
...
static int parse_all_stubs(const char *stdlib_dir)
{
    char path[512];
    int i;
    static const char *function_files[] = {
        "funcs.py", "dosio.py", "dostui.py", 0
    };
    static const char *type_files[] = {
        "str.py", "list.py", "dict.py", "set.py", "int.py", "float.py", "tuple.py",
        "frozenst.py", "complex.py", "bytearr.py", "bytes.py", "range.py", 0
    };

    for (i = 0; function_files[i]; i++) {
        snprintf(path, sizeof(path), "%s/builtins/%s", stdlib_dir,
                 function_files[i]);
...
/* ================================================================= */
/* Pass 2: Compile a Python stdlib source file to PIR                   */
/* ================================================================= */

static PIRModule *compile_python_source(const char *source_path,
                                        StdlibRegistry *temp_reg)
{
    ...
    error_init(source_path);
    types_init();

    lexer = new Lexer();
    if (!lexer->open(source_path)) {
        fprintf(stderr, "stdbld: cannot open %s\n", source_path);
        delete lexer;
        types_shutdown();
        error_shutdown();
    ...
    if (parser->get_error_count() > 0) {
        fprintf(stderr, "stdbld: %d parse error(s) in %s\n",
                parser->get_error_count(), source_path);
        delete parser;
        delete lexer;
        ast_free_all();
        types_shutdown();
    ...
    if (sema->get_error_count() > 0) {
        fprintf(stderr, "stdbld: %d sema error(s) in %s\n",
                sema->get_error_count(), source_path);
        delete sema;
        delete parser;
        delete lexer;
        ast_free_all();
...
/* Transfer all class/function bodies except the source module initializer. */
static void merge_compiled_module(PIRModule *dst, PIRModule *src)
{
    int i;
    if (!dst || !src) return;

    for (i = 0; i < src->functions.size(); i++) {
        PIRFunction *func = src->functions[i];
        if (!func) continue;
        if (func == src->init_func ||
            (func->name && strcmp(func->name, "__init__") == 0)) {
            continue;
        }
        dst->functions.push_back(func);
        src->functions[i] = 0;
    }
...
static const char *type_source_file(const char *type_name)
{
    if (!type_name) return 0;
    if (strcmp(type_name, "frozenset") == 0) return "frozenst.py";
    if (strcmp(type_name, "bytearray") == 0) return "bytearr.py";
    if (strcmp(type_name, "bytes") == 0) return "bytes.py";
    if (strcmp(type_name, "str") == 0) return "str.py";
    if (strcmp(type_name, "list") == 0) return "list.py";
    if (strcmp(type_name, "dict") == 0) return "dict.py";
    if (strcmp(type_name, "set") == 0) return "set.py";
    if (strcmp(type_name, "int") == 0) return "int.py";
    if (strcmp(type_name, "float") == 0) return "float.py";
    if (strcmp(type_name, "tuple") == 0) return "tuple.py";
    if (strcmp(type_name, "complex") == 0) return "complex.py";
    if (strcmp(type_name, "range") == 0) return "range.py";
    return 0;
}
                              ...
                              PIRFunction **out_funcs, int out_cap)
{
    ...
    /* Seed: all is_pir methods (named TYPE__METHOD in PIR module) */
    {
        int ti, mi;
                 ...
                 const char *output_path)
{
    ...
    /* Discover every Python-backed builtin and class method. */
    for (i = 0; i < g_num_funcs; i++) {
        if (g_funcs[i].is_pir) {
            has_pir_funcs = 1;
    ...
    for (i = 0; i < g_num_types; i++) {
        for (j = 0; j < g_types[i].num_methods; j++) {
            if (g_types[i].methods[j].is_pir) {
                has_pir_methods = 1;
                printf("  pir: %s.%s (params=%d)\n",
                       g_types[i].type_name,
                       g_types[i].methods[j].method_name,
                       g_types[i].methods[j].num_params);
            }
        }
    }

    /* Compile module-level functions and class methods from their sources. */
    if (has_pir_funcs || has_pir_methods) {
        StdlibRegistry *temp_reg = build_temp_registry();
        if (!temp_reg) {
            fprintf(stderr, "stdbld: failed to build temporary registry\n");
            return 1;
        }
        ...
        if (has_pir_funcs) {
            ...
            test = fopen(pyfncs_path, "r");
            if (!test) {
                fprintf(stderr, "stdbld: %s not found\n", pyfncs_path);
                delete temp_reg;
                return 1;
            }
            fclose(test);

            pir_mod = compile_python_source(pyfncs_path, temp_reg);
            if (!pir_mod) {
        ...
        if (!pir_mod) pir_mod = pir_module_new();
        ...
        for (i = 0; i < g_num_types; i++) {
            ...
            if (!type_has_pir) continue;

            file_name = type_source_file(g_types[i].type_name);
            if (!file_name) {
                fprintf(stderr, "stdbld: no source file for type %s\n",
                        g_types[i].type_name);
                delete temp_reg;
            ...
            snprintf(source_path, sizeof(source_path), "%s/builtins/%s",
                     stdlib_dir, file_name);
            type_mod = compile_python_source(source_path, temp_reg);
            if (!type_mod) {
                fprintf(stderr, "stdbld: failed to compile %s\n", source_path);
                delete temp_reg;
                pir_module_free(pir_mod);
            ...
            printf("stdbld: compiled Python methods from %s\n", source_path);
            merge_compiled_module(pir_mod, type_mod);
        }
    ...
    if (pir_mod) {
        ...
        for (i = 0; i < g_num_types; i++) {
            for (j = 0; j < g_types[i].num_methods; j++) {
                if (g_types[i].methods[j].is_pir) pc++;
            }
        }
        printf(", %d pir", pc);
    }
