                    ...
                    if (specialized) {
                        ...
                        if (entry) {
                            ...
                            if (specialized->kind == AST_CLASS_DEF) {
                                ...
                                specialized->data.class_def.generic_origin =
                                    mono_str_dup(gname);
                            }
                            entry->next = entries;
                            entries = entry;
                            if (sema) {
                                int before = sema->get_error_count();
                                sema->analyze_fragment(specialized);
                                error_count += sema->get_error_count() - before;
                            }
                        }
                    }
                }
...
void Monomorphizer::inject_and_rewrite(ASTNode *module)
{
    ...
    /* 1. Replace every generic class definition with its specializations at
     *    the same position.  Moving them to the front of the module would put
     *    a class ahead of the definitions its decorators need, so a decorated
     *    method would apply staticmethod or property before those classes
     *    exist. */
    ...
    while (child) {
        if (child->kind == AST_CLASS_DEF && child->data.class_def.name &&
            (is_generic_class(child->data.class_def.name) ||
             child->data.class_def.num_type_params > 0)) {
            ASTNode *following = child->next;
            for (e = entries; e; e = e->next) {
                if (!e->specialized_ast || e->inserted) continue;
                if (!e->generic_name ||
                    strcmp(e->generic_name, child->data.class_def.name) != 0)
                    continue;
                e->inserted = 1;
                *prev_ptr = e->specialized_ast;
                prev_ptr = &e->specialized_ast->next;
            }
            *prev_ptr = following;
            child = following;
        } else {
    ...
    /* 2. A specialization whose generic class is not in this module body
     *    keeps the previous placement. */
    for (e = entries; e; e = e->next) {
        if (e->specialized_ast && !e->inserted) {
            e->inserted = 1;
...
void Monomorphizer::rewrite_references(ASTNode *node)
{
    ...
    case AST_CLASS_DEF:
    {
        ...
        if (node->data.class_def.metaclass)
            rewrite_references(node->data.class_def.metaclass);
        for (child = node->data.class_def.keywords; child;
             child = child->next)
            rewrite_references(child);
...
static void type_mangle_append(TypeInfo *t, char *buf, int *pos, int max)
{
    ...
    case TY_COMPLEX: *pos = buf_append(buf, *pos, max, "complex"); break;
    case TY_BYTEARRAY: *pos = buf_append(buf, *pos, max, "bytearray"); break;
    ...
    case TY_FROZENSET:
        *pos = buf_append(buf, *pos, max, "frozenset_");
        if (t->type_args) type_mangle_append(t->type_args, buf, pos, max);
        else *pos = buf_append(buf, *pos, max, "any");
        break;
                                   ...
                                   TypeInfo **concrete_types, int num_params)
{
    ...
    case AST_FUNC_DEF:
        ...
        c->data.func_def.type_param_bounds = node->data.func_def.type_param_bounds;
        c->data.func_def.type_param_kinds = node->data.func_def.type_param_kinds;
    ...
    case AST_CLASS_DEF:
        ...
        c->data.class_def.metaclass = copy_node(
            node->data.class_def.metaclass, param_names,
            concrete_types, num_params);
        c->data.class_def.keywords = 0;
        prev = 0;
        for (child = node->data.class_def.keywords; child;
             child = child->next) {
            cpy = copy_node(child, param_names, concrete_types, num_params);
            if (cpy) {
                cpy->next = 0;
                if (prev) prev->next = cpy;
                else c->data.class_def.keywords = cpy;
                prev = cpy;
            }
        }
        /* Copy body */
        ...
        c->data.class_def.type_param_names = node->data.class_def.type_param_names;
        c->data.class_def.type_param_bounds = node->data.class_def.type_param_bounds;
        c->data.class_def.type_param_kinds = node->data.class_def.type_param_kinds;
        c->data.class_def.num_type_params = node->data.class_def.num_type_params;
        break;
    ...
    case AST_LAMBDA:
        ...
        c->data.lambda.body = copy_node(node->data.lambda.body, param_names,
                                          concrete_types, num_params);
        c->data.lambda.num_free_vars = node->data.lambda.num_free_vars;
        c->data.lambda.free_var_names = node->data.lambda.free_var_names;
        break;
        ...
        if (node->data.type_alias.num_type_params > 0) {
            ...
            c->data.type_alias.type_param_names =
                (const char **)malloc(node->data.type_alias.num_type_params *
                                      sizeof(const char *));
            c->data.type_alias.type_param_bounds =
                (ASTNode **)calloc(node->data.type_alias.num_type_params,
                                   sizeof(ASTNode *));
            c->data.type_alias.type_param_kinds =
                (unsigned char *)calloc(node->data.type_alias.num_type_params,
                                        1);
            for (i = 0; i < node->data.type_alias.num_type_params; i++) {
                ...
                if (c->data.type_alias.type_param_bounds &&
                    node->data.type_alias.type_param_bounds)
                    c->data.type_alias.type_param_bounds[i] = copy_node(
                        node->data.type_alias.type_param_bounds[i],
                        param_names, concrete_types, num_params);
                if (c->data.type_alias.type_param_kinds &&
                    node->data.type_alias.type_param_kinds)
                    c->data.type_alias.type_param_kinds[i] =
                        node->data.type_alias.type_param_kinds[i];
            }
        } else {
            c->data.type_alias.type_param_names = 0;
            c->data.type_alias.type_param_bounds = 0;
            c->data.type_alias.type_param_kinds = 0;
        }
...
void Monomorphizer::scan_for_instantiations(ASTNode *node, ASTNode *module)
{
    ...
    case AST_CLASS_DEF:
        ...
        if (node->data.class_def.metaclass)
            scan_for_instantiations(node->data.class_def.metaclass, module);
        for (child = node->data.class_def.keywords; child;
             child = child->next)
            scan_for_instantiations(child, module);
        break;
