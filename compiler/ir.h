...
enum IROp {
    ...
    IR_GUARDED_CALL_METHOD, /* CALL_METHOD with guarded direct target */
    ...
    IR_CHECK_EXCEPTION, /* if exception pending: jump to extra */
    IR_CLEAR_EXCEPTION, /* clear pending exception */
    IR_PANIC_EXCEPTION, /* report pending exception and terminate */
    ...
    IR_MAKE_FUNCTION,   /* src1 = code-label name, extra = Python-visible name */
...
struct IRInstr {
    ...
    int         line;           /* originating Python source line */
    const char *guard_target;   /* direct symbol for guarded method calls */
...
struct ClassVTableInfo {
    ...
    /* Python-visible name.  A monomorphized generic keeps its original name
     * here while class_name carries the mangled symbol. */
...
struct IRModule {
    ...
    ClassVTableInfo class_vtables[128];
