 ...
 *   - DS is restored to DGROUP immediately after every call
...
CodeGenerator8086::CodeGenerator8086()
{
    ...
    scope_temp_count = 0;
    loop_scope_tracks = 0;
    loop_scope_track_count = 0;
    scope_map_id = -1;
    next_scope_map_id = 0;
    exception_stub_handlers = 0;
    exception_stub_labels = 0;
    exception_stub_lines = 0;
    exception_stub_count = 0;
    exception_stub_capacity = 0;
}

CodeGenerator8086::~CodeGenerator8086()
{
    free(scope_temps);
    free(loop_scope_tracks);
    free(exception_stub_handlers);
    free(exception_stub_labels);
    free(exception_stub_lines);
}
...
/* Calls are followed automatically by the smallest possible DS restoration
 * in note_call_emitted().  Existing semantic markers therefore emit no code. */
void CodeGenerator8086::emit_restore_ds()
{
}

void CodeGenerator8086::note_call_emitted()
{
    /* Write directly to avoid recursively notifying emit_line(). */
    fprintf(out, "    push ss\n");
    fprintf(out, "    pop  ds\n");
}
...
void CodeGenerator8086::store_dxax_to_temp(int temp)
{
    ...
    int lo;
    int hi;
    /* No destination means the result is discarded.  Slot arithmetic on a
     * negative temp would land on a local and overwrite it. */
    if (temp < 0) return;
    slot = current_func->num_locals + temp;
    lo = slot_offset_lo(slot);
    hi = slot_offset_hi(slot);
...
{
    int i;
    /* String literals are immutable and every reference emitted by this
     * backend already carries an explicit segment and offset.  Keeping them
     * in DGROUP needlessly consumes the single 64 KiB near-data group shared
     * by writable globals, the C runtime and the stack.  Put them in a far
     * data segment instead; this is exactly what the LARGE memory model is
     * designed for. */
    fprintf(out, ".FARDATA PYCONST\n");
    emit_blank();
    ...
    /* Method names are also immutable and are always passed as far
     * pointers, so they do not belong in DGROUP either. */
    fprintf(out, ".FARDATA PYMETHOD\n");
    emit_blank();
    ...
    /* Writable process state remains in DGROUP and is addressable through
     * DS after calls into the C runtime. */
    fprintf(out, ".DATA\n");
    emit_blank();
...
void CodeGenerator8086::emit_code_section()
{
    ...
    for (f = mod->functions; f; f = f->next) {
        ...
        /* Real-mode segments are limited to 64 KiB.  Keeping each generated
         * function in its own code segment prevents large modules from
         * producing a corrupt OMF object when their combined code exceeds
         * that limit. */
...
void CodeGenerator8086::emit_func(IRFunc *func)
{
    ...
    exception_stub_count = 0;
    exception_stub_capacity = 0;
    for (scope_scan = func->first; scope_scan; scope_scan = scope_scan->next) {
        if (scope_scan->op == IR_CHECK_EXCEPTION && scope_scan->src1 >= 0)
            exception_stub_capacity++;
    }
    if (exception_stub_capacity > 0) {
        exception_stub_handlers = (int *)malloc(
            sizeof(int) * exception_stub_capacity);
        exception_stub_labels = (int *)malloc(
            sizeof(int) * exception_stub_capacity);
        exception_stub_lines = (int *)malloc(
            sizeof(int) * exception_stub_capacity);
        if (!exception_stub_handlers || !exception_stub_labels ||
            !exception_stub_lines) {
            fprintf(stderr, "codegen: out of memory (exception stubs)\n");
            exit(1);
        }
    }
    ...
    loop_scope_tracks = 0;
    loop_scope_track_count = 0;
    scope_map_id = next_scope_map_id++;

    track_count = 0;
    max_label = -1;
    for (scope_scan = func->first; scope_scan; scope_scan = scope_scan->next) {
        if (scope_scan->op == IR_SCOPE_TRACK) track_count++;
        if (scope_scan->op == IR_LABEL && scope_scan->extra > max_label)
            max_label = scope_scan->extra;
    }
    ...
    label_positions = 0;
    if (track_count > 0) {
        all_tracks = (IRInstr **)malloc(sizeof(IRInstr *) * track_count);
        track_positions = (int *)malloc(sizeof(int) * track_count);
        track_in_loop = (unsigned char *)calloc(track_count, 1);
        scope_temps = (int *)malloc(sizeof(int) * track_count);
        loop_scope_tracks = (IRInstr **)malloc(sizeof(IRInstr *) * track_count);
    }
    if (max_label >= 0)
        label_positions = (int *)malloc(sizeof(int) * (max_label + 1));
    if ((track_count > 0 && (!all_tracks || !track_positions ||
         !track_in_loop || !scope_temps || !loop_scope_tracks)) ||
        (max_label >= 0 && !label_positions)) {
        fprintf(stderr, "codegen: out of memory (scope frame map)\n");
        exit(1);
    }
    for (ti = 0; ti <= max_label; ti++) label_positions[ti] = -1;

    position = 0;
    ti = 0;
    for (scope_scan = func->first; scope_scan;
         scope_scan = scope_scan->next, position++) {
        if (scope_scan->op == IR_LABEL && scope_scan->extra >= 0)
            label_positions[scope_scan->extra] = position;
        if (scope_scan->op == IR_SCOPE_TRACK) {
            all_tracks[ti] = scope_scan;
            track_positions[ti] = position;
            ti++;
        }
    }
    ...
    for (scope_scan = func->first; scope_scan;
         scope_scan = scope_scan->next, position++) {
        if ((scope_scan->op == IR_JUMP ||
             scope_scan->op == IR_JUMP_IF_TRUE ||
             scope_scan->op == IR_JUMP_IF_FALSE) &&
            scope_scan->extra >= 0 && scope_scan->extra <= max_label &&
            label_positions[scope_scan->extra] >= 0 &&
            label_positions[scope_scan->extra] <= position) {
            int loop_start = label_positions[scope_scan->extra];
            for (ti = 0; ti < track_count; ti++) {
                if (track_positions[ti] >= loop_start &&
                    track_positions[ti] <= position)
                    track_in_loop[ti] = 1;
            }
        }
    ...
    for (ti = 0; ti < track_count; ti++) {
        if (track_in_loop[ti])
            loop_scope_tracks[loop_scope_track_count++] = all_tracks[ti];
        else
            scope_temps[scope_temp_count++] = all_tracks[ti]->src1;
    }
    free(all_tracks);
    free(track_positions);
    free(track_in_loop);
    ...
    if (func != mod->init_func) {
        int monitoring_skip = new_label();
        emit_line("cmp  byte ptr pydos_monitoring_active_, 0");
        emit_line("je   _L%d", monitoring_skip);
        emit_line("mov  ax, seg %s", flbl);
        emit_line("push ax");
        emit_line("mov  ax, offset %s", flbl);
        emit_line("push ax");
        emit_restore_ds();
        emit_line("call far ptr pydos_monitoring_py_start_");
        emit_line("add  sp, 4");
        emit_restore_ds();
        emit_local_label(monitoring_skip);
    }

    if (func == mod->init_func) {
        emit_builtin_pir_method_registrations();
    }
    ...
    if (scope_temp_count > 0) {
        int i;
        fprintf(out, "_SM%d LABEL WORD\n", scope_map_id);
        for (i = 0; i < scope_temp_count; i++) {
            int slot = func->num_locals + scope_temps[i];
            fprintf(out, "    dw %d\n", slot_offset_lo(slot));
        }
    }
...
void CodeGenerator8086::emit_builtin_pir_method_registrations()
{
    int i;

    for (i = 0; i < num_builtin_pir_methods; i++) {
        BuiltinPIRMethodBinding *binding = &builtin_pir_methods[i];
        char func_lbl[128];
        char mn_lbl[32];

        func_label(func_lbl, sizeof(func_lbl), binding->func_name);
        sprintf(mn_lbl, "_MN_%d", binding->mn_string_index);
        emit_comment("register builtin method type=%d '%s' -> %s",
                     binding->runtime_type_tag, binding->method_name, func_lbl);

        /* cdecl arguments, right to left: far func, far name, type tag. */
        emit_line("mov  ax, seg %s", func_lbl);
        emit_line("push ax");
        emit_line("mov  ax, offset %s", func_lbl);
        emit_line("push ax");
        emit_push_far_const(mn_lbl);
        emit_line("mov  ax, %d", binding->runtime_type_tag);
        emit_line("push ax");
        emit_restore_ds();
        emit_line("call far ptr pydos_builtin_vtable_add_method_");
        emit_line("add  sp, 10");
        emit_restore_ds();
    }

    if (num_builtin_pir_methods > 0) emit_blank();
}
...
void CodeGenerator8086::emit_prologue(IRFunc *func)
{
    ...
    emit_line("push si");
    emit_line("push di");
    /* A generated function may be entered through a runtime callback whose
     * DS reflects the caller.  Establish the program DGROUP before touching
     * globals or invoking another function. */
    emit_line("push ss");
    emit_line("pop  ds");
...
void CodeGenerator8086::emit_const_str(IRInstr *instr)
{
    ...
    /* String data may contain CR/LF or other assembler-significant bytes. */
    emit_comment("CONST_STR len=%d -> t%d", slen, instr->dest);
...
void CodeGenerator8086::emit_load_global(IRInstr *instr)
{
    ...
    if (!is_registered_global(gname) && stdlib_reg_ &&
        stdlib_reg_->is_pir_backed(gname)) {
        int name_idx = register_mn_string(gname);
        int dst_slot = current_func->num_locals + instr->dest;
        int arg_count = 0;
        int signature_idx;
        const char *signature = stdlib_reg_->find_pir_signature(gname);
        char name_label[32];
        char signature_label[32];
        char function_label[128];
        IRFunc *defined;
        sprintf(name_label, "_MN_%d", name_idx);
        signature_idx = register_mn_string(signature ? signature : "");
        sprintf(signature_label, "_MN_%d", signature_idx);
        func_label(function_label, sizeof(function_label), gname);
        for (defined = mod->functions; defined; defined = defined->next) {
            if (defined->name && strcmp(defined->name, gname) == 0) {
                arg_count = defined->num_params;
                break;
            }
        }
        require_extern("pydos_func_new_");
        require_extern("pydos_func_set_signature_");
        emit_comment("LOAD_GLOBAL (PIR function object) '%s' -> t%d",
                     gname, instr->dest);
        emit_push_far_const(name_label);
        emit_line("mov  ax, seg %s", function_label);
        emit_line("push ax");
        emit_line("mov  ax, offset %s", function_label);
        emit_line("push ax");
        emit_restore_ds();
        emit_line("call far ptr pydos_func_new_");
        emit_line("add  sp, 8");
        emit_restore_ds();
        emit_line("mov  word ptr [bp%+d], ax", slot_offset_lo(dst_slot));
        emit_line("mov  word ptr [bp%+d], dx", slot_offset_hi(dst_slot));
        emit_push_far_const(signature_label);
        emit_push_imm(arg_count);
        emit_line("push word ptr [bp%+d]", slot_offset_hi(dst_slot));
        emit_line("push word ptr [bp%+d]", slot_offset_lo(dst_slot));
        emit_restore_ds();
        emit_line("call far ptr pydos_func_set_signature_");
        emit_line("add  sp, 10");
        emit_restore_ds();
        return;
    }
    ...
    if (!is_registered_global(gname) && builtin_asm_name(gname)) {
        int type_tag = stdlib_reg_
                       ? stdlib_reg_->find_runtime_type_tag(gname) : -1;
        if (type_tag < 0 && strcmp(gname, "type") == 0)
            type_tag = 14; /* PYDT_CLASS */
        if (type_tag >= 0) {
            int dst_slot = current_func->num_locals + instr->dest;
            require_extern("pydos_builtin_type_object_");
            emit_comment("LOAD_GLOBAL (type) '%s' -> t%d", gname,
                         instr->dest);
            emit_push_imm(type_tag);
            emit_restore_ds();
            emit_line("call far ptr pydos_builtin_type_object_");
            emit_line("add  sp, 2");
            emit_restore_ds();
            emit_line("mov  word ptr [bp%+d], ax",
                      slot_offset_lo(dst_slot));
            emit_line("mov  word ptr [bp%+d], dx",
                      slot_offset_hi(dst_slot));
        } else {
            int name_idx = register_mn_string(gname);
            int dst_slot = current_func->num_locals + instr->dest;
            char name_label[32];
            ...
            require_extern(builtin_asm_name(gname));
            emit_comment("LOAD_GLOBAL (builtin object) '%s' -> t%d",
                         gname, instr->dest);
            emit_push_far_const(name_label);
            emit_line("mov  ax, seg %s", builtin_asm_name(gname));
            emit_line("push ax");
            emit_line("mov  ax, offset %s", builtin_asm_name(gname));
            emit_line("push ax");
            emit_restore_ds();
            emit_line("call far ptr pydos_func_new_builtin_");
            emit_line("add  sp, 8");
            emit_restore_ds();
            emit_line("mov  word ptr [bp%+d], ax",
                      slot_offset_lo(dst_slot));
...
void CodeGenerator8086::emit_arithmetic(IRInstr *instr)
{
    ...
    const char *func_name = arith_dispatch_func(instr);
...
void CodeGenerator8086::emit_call(IRInstr *instr)
{
    ...
    if (callee_name && !is_registered_global(callee_name)) {
    ...
    if (builtin_name) {
        ...
        if (is_direct) {
            ...
            push_temp(instr->src1);
            emit_restore_ds();
            emit_line("call far ptr pydos_obj_call_");
            emit_line("add  sp, %d", 10 + argc * 4);
            emit_restore_ds();
            store_dxax_to_temp(instr->dest);
            num_call_args = 0;
            return;
...
void CodeGenerator8086::emit_call_method(IRInstr *instr)
{
    ...
    require_extern(instr->op == IR_GUARDED_CALL_METHOD
    ...
    if (instr->op == IR_GUARDED_CALL_METHOD) {
        char direct_lbl[160];
        func_label(direct_lbl, sizeof(direct_lbl), instr->guard_target);
        emit_line("mov  ax, seg %s", direct_lbl);
        emit_line("push ax");
        emit_line("mov  ax, offset %s", direct_lbl);
        emit_line("push ax");
    }
    ...
    emit_restore_ds();
    emit_line(instr->op == IR_GUARDED_CALL_METHOD
              ? "call far ptr pydos_obj_call_method_guarded_"
              : "call far ptr pydos_obj_call_method_");
    /* name(4) + optional target(4) + argc(2) + argv(4) + argv array */
    emit_line("add  sp, %d",
              (instr->op == IR_GUARDED_CALL_METHOD ? 14 : 10) +
              total_argc * 4);
    emit_restore_ds();
...
void CodeGenerator8086::emit_return(IRInstr *instr)
{
    ...
    if (instr->src1 >= 0) {
        ...
        /* Every non-NULL Python return crosses the ABI as an owned
         * reference.  Scope insertion already promotes tracked results and
         * marks them through extra; all other returns are borrowed loads
         * from parameters, cells, locals or globals and must be promoted
         * here, including functions with no parameters. */
        if (!instr->extra) {
            emit_line("push dx");
            emit_line("push ax");
            emit_restore_ds();
    ...
    if (current_func != mod->init_func) {
        char flbl[128];
        int monitoring_skip = new_label();
        func_label(flbl, sizeof(flbl), current_func->name);
        emit_line("cmp  byte ptr pydos_monitoring_active_, 0");
        emit_line("je   _L%d", monitoring_skip);
        emit_line("push dx");
        emit_line("push ax");
        ...
        emit_line("push ax");
        emit_restore_ds();
        emit_line("call far ptr pydos_monitoring_py_return_");
        emit_line("add  sp, 8");
        emit_restore_ds();
        if (instr->src1 >= 0)
            load_temp_to_dxax(instr->src1);
...
void CodeGenerator8086::emit_build_tuple(IRInstr *instr)
{
    ...
    if (count == 0) {
        ...
        emit_line("call far ptr pydos_obj_new_empty_tuple_");
        store_dxax_to_temp(instr->dest);
        num_call_args = 0;
        return;
    }
    ...
    push_temp(instr->dest);
    emit_restore_ds();
    emit_line("call far ptr pydos_list_to_tuple_");
    emit_line("add  sp, 4");
    emit_restore_ds();
    store_dxax_to_temp(instr->dest);
...
void CodeGenerator8086::emit_for_iter(IRInstr *instr)
{
    ...
    /* Distinguish normal exhaustion from an iterator exception. */
    store_dxax_to_temp(instr->dest);
    emit_line("mov  cx, ax");
    emit_line("or   cx, dx");
    {
        int has_value = new_label();
        emit_line("jnz  _L%d", has_value);
        emit_restore_ds();
        emit_line("call far ptr pydos_exc_pending_");
        emit_restore_ds();
        emit_line("test ax, ax");
        emit_line("jnz  _L%d", instr->src2);
        emit_line("jmp  _L%d", instr->extra);
        emit_local_label(has_value);
    }
}
...
void CodeGenerator8086::emit_load_subscript(IRInstr *instr)
{
    ...
    if (sub_fn && instr->type_hint &&
        ...
        /* Dict operator ABI: fn(dict, key_obj). */
        require_extern(sub_fn);
...
{
    /* Handler edges are explicit CHECK_EXCEPTION instructions. */
...
{
    int name_idx = register_mn_string(current_func->name);
    /* IR_RAISE: raise src1 */
    ...
    emit_line("call far ptr pydos_exc_add_traceback_");
    emit_line("add  sp, 6");
    emit_restore_ds();
    emit_line("jmp  _L%d", instr->extra);
}
...
void CodeGenerator8086::emit_get_exception(IRInstr *instr)
{
    ...
    emit_restore_ds();
    emit_line("call far ptr pydos_exc_fetch_");
    emit_restore_ds();

    store_dxax_to_temp(instr->dest);
}

void CodeGenerator8086::emit_check_exception(IRInstr *instr)
{
    emit_comment("CHECK_EXCEPTION -> _L%d", instr->extra);
    if (instr->src1 >= 0) {
        int stub_label = get_exception_stub(instr->extra, instr->line);
        int continue_label = new_label();
        load_temp_to_dxax(instr->src1);
        emit_line("mov  bx, ax");
        emit_line("or   bx, dx");
        emit_line("jnz  _LCG%d", continue_label);
        emit_line("jmp  _LCG%d", stub_label);
        emit_line("_LCG%d:", continue_label);
        return;
    }
    {
        int name_idx = register_mn_string(current_func->name);
        ...
        sprintf(name_label, "_MN_%d", name_idx);
        emit_push_far_const(name_label);
        emit_line("call far ptr pydos_exc_check_traceback_");
        emit_line("add  sp, 4");
        emit_line("test ax, ax");
        emit_line("jnz  _L%d", instr->extra);
    }
...
int CodeGenerator8086::get_exception_stub(int handler_label, int line)
{
    int i;
    for (i = 0; i < exception_stub_count; i++) {
        if (exception_stub_handlers[i] == handler_label &&
            exception_stub_lines[i] == line)
            return exception_stub_labels[i];
    }
    if (exception_stub_count >= exception_stub_capacity) {
        fprintf(stderr, "codegen: exception stub capacity exceeded\n");
        exit(1);
    }
    exception_stub_handlers[exception_stub_count] = handler_label;
    exception_stub_lines[exception_stub_count] = line;
    exception_stub_labels[exception_stub_count] = new_label();
    return exception_stub_labels[exception_stub_count++];
}

void CodeGenerator8086::emit_exception_stubs(IRFunc *func)
{
    int i;
    int name_idx;
    char name_label[32];
    if (exception_stub_count == 0) return;

    emit_line("jmp  _%s_epilogue", func->name);
    name_idx = register_mn_string(func->name);
    sprintf(name_label, "_MN_%d", name_idx);
    for (i = 0; i < exception_stub_count; i++) {
        emit_line("_LCG%d:", exception_stub_labels[i]);
        emit_push_imm(exception_stub_lines[i]);
        emit_push_far_const(name_label);
        emit_line("call far ptr pydos_exc_require_traceback_");
        emit_line("add  sp, 6");
        emit_line("jmp  _L%d", exception_stub_handlers[i]);
    }
}

void CodeGenerator8086::emit_clear_exception(IRInstr *instr)
{
    (void)instr;
    emit_comment("CLEAR_EXCEPTION");
    require_extern("pydos_exc_clear_");
    emit_restore_ds();
    emit_line("call far ptr pydos_exc_clear_");
    emit_restore_ds();
}

void CodeGenerator8086::emit_panic_exception(IRInstr *instr)
{
    (void)instr;
    emit_comment("PANIC_EXCEPTION");
    require_extern("pydos_exc_panic_current_");
    emit_restore_ds();
    emit_line("call far ptr pydos_exc_panic_current_");
    emit_restore_ds();
}
...
void CodeGenerator8086::emit_init_vtable(IRInstr *instr)
{
    ...
    char vt_label[256];
    vtable_label(vt_label, sizeof(vt_label), vti->class_name);

    /* Reserve exactly the number of declared methods.  The fixed slot array
     * remains in the vtable, while general-method metadata stays compact. */
    require_extern("pydos_vtable_create_sized_");
    emit_push_imm(vti->num_methods);
    emit_restore_ds();
    emit_line("call far ptr pydos_vtable_create_sized_");
    emit_line("add  sp, 2");
    emit_restore_ds();
    ...
    {
        int cn_idx = register_mn_string(vti->display_name);
        char cn_lbl[32];
        sprintf(cn_lbl, "_MN_%d", cn_idx);

        require_extern("pydos_vtable_set_name_");
        emit_comment("  set class name '%s'", vti->display_name);
    ...
    for (mi = 0; mi < vti->num_methods; mi++) {
        ...
        /* Push fixed generated ABI arity (rightmost parameter). */
        emit_push_imm(vti->methods[mi].arg_count);
        ...
        emit_restore_ds();
        emit_line("call far ptr pydos_vtable_add_method_sig_");
        emit_line("add  sp, 14");
        emit_restore_ds();
    }
    ...
    if (vti->base_class_name && vti->base_class_name[0] != '\0' &&
        (!stdlib_reg_ || stdlib_reg_->find_runtime_type_tag(
             vti->base_class_name) < 0)) {
        require_extern("pydos_vtable_inherit_");

        char pvt_label[256];
        vtable_label(pvt_label, sizeof(pvt_label), vti->base_class_name);
        ...
        /* Inherit from additional bases (multiple inheritance) */
        {
            ...
            for (ebi = 0; ebi < vti->num_extra_bases; ebi++) {
                char eb_label[256];
                if (stdlib_reg_ && stdlib_reg_->find_runtime_type_tag(
                        vti->extra_bases[ebi]) >= 0)
                    continue;
                vtable_label(eb_label, sizeof(eb_label), vti->extra_bases[ebi]);
    ...
    /* Materialize the Python class object and bind it to the class global. */
    {
        const char *class_global = global_asm_name(vti->class_name);
        int cn_idx = register_mn_string(vti->display_name);
        char cn_lbl[32];
        int bi;
        sprintf(cn_lbl, "_MN_%d", cn_idx);
        ...
        emit_push_far_const(cn_lbl);
        emit_restore_ds();
        emit_line("call far ptr pydos_class_new_");
        emit_line("add  sp, 8");
        emit_restore_ds();
        emit_line("mov  word ptr [%s], ax", class_global);
        emit_line("mov  word ptr [%s+2], dx", class_global);

        if ((!vti->base_class_name || vti->base_class_name[0] == '\0') &&
            vti->num_extra_bases == 0) {
            emit_line("push word ptr [%s+2]", class_global);
            emit_line("push word ptr [%s]", class_global);
            emit_restore_ds();
            emit_line("call far ptr pydos_class_add_object_base_");
            emit_line("add  sp, 4");
            emit_restore_ds();
        }

        for (bi = 0; bi < 1 + vti->num_extra_bases; bi++) {
            const char *base = bi == 0 ? vti->base_class_name
                                      : vti->extra_bases[bi - 1];
            const char *base_global;
            int runtime_tag;
            if (!base || base[0] == '\0') continue;
            runtime_tag = stdlib_reg_
                          ? stdlib_reg_->find_runtime_type_tag(base) : -1;
            if (runtime_tag >= 0) {
                require_extern("pydos_builtin_type_object_");
                emit_push_imm(runtime_tag);
                emit_restore_ds();
                emit_line("call far ptr pydos_builtin_type_object_");
                emit_line("add  sp, 2");
                emit_restore_ds();
                ...
                emit_line("push word ptr [%s]", class_global);
                emit_restore_ds();
                emit_line("call far ptr pydos_class_add_base_");
                emit_line("add  sp, 8");
                emit_restore_ds();
                continue;
            }
            base_global = global_asm_name(base);
            emit_line("push word ptr [%s+2]", base_global);
            emit_line("push word ptr [%s]", base_global);
            emit_line("push word ptr [%s+2]", class_global);
            emit_line("push word ptr [%s]", class_global);
            emit_restore_ds();
            emit_line("call far ptr pydos_class_add_base_");
...
void CodeGenerator8086::emit_set_vtable(IRInstr *instr)
{
    ...
    /* Preserve the runtime identity of the concrete class on the instance. */
    {
        const char *class_global = global_asm_name(vti->class_name);
        ...
        push_temp(instr->src1);
        emit_restore_ds();
        emit_line("call far ptr pydos_obj_set_class_");
        emit_line("add  sp, 8");
        emit_restore_ds();
    }
}
...
void CodeGenerator8086::emit_check_vtable(IRInstr *instr)
{
    ...
    if (vt_idx < 0 || vt_idx >= mod->num_class_vtables) {
        ...
        /* Return False.  "push imm" is 186+, so the 8086 needs a register. */
        require_extern("pydos_obj_new_bool_");
        emit_line("xor  ax, ax");
        emit_line("push ax");
        emit_restore_ds();
...
{
    /* IR_MAKE_FUNCTION: src1 identifies the assembly label while extra is
     * the Python-visible name.  Keeping both in static storage avoids a
     * permanent heap string for __name__ and code.co_name. */
    int name_ci = instr->src1;
    int display_ci = instr->extra >= 0 ? (int)instr->extra : name_ci;
    const char *fname = "";
    if (name_ci >= 0 && name_ci < mod->num_constants &&
        mod->constants[name_ci].kind == IRConst::CONST_STR) {
    ...
    {
        char slbl[64];
        const_label(slbl, sizeof(slbl), display_ci);
    ...
    /* Preserve the generated fixed ABI arity on first-class functions. */
    {
        IRFunc *defined;
        int arg_count = -1;
        for (defined = mod->functions; defined; defined = defined->next) {
            if (defined->name && strcmp(defined->name, fname) == 0) {
                arg_count = defined->num_params;
                break;
            }
        }
        ...
        if (arg_count >= 0) {
            ...
            push_temp(instr->dest);
            emit_restore_ds();
            emit_line("call far ptr pydos_func_set_arg_count_");
            emit_line("add  sp, 6");
            emit_restore_ds();
        }
        load_temp_to_dxax(instr->dest);
...
{
    (void)instr;
    emit_comment("SCOPE_ENTER (frame map%s)",
                 loop_scope_track_count > 0 ? " + loop ledger" : "");
    if (loop_scope_track_count > 0)
...
{
    int i;
    for (i = 0; i < loop_scope_track_count; i++) {
        if (loop_scope_tracks[i] == instr) {
            int slot = current_func->num_locals + instr->src1;
            emit_comment("SCOPE_TRACK t%d (loop ledger)", instr->src1);
            emit_line("push word ptr [bp%+d]", slot_offset_hi(slot));
            emit_line("push word ptr [bp%+d]", slot_offset_lo(slot));
            emit_line("call far ptr %s", instr->extra
    ...
    emit_comment("SCOPE_TRACK t%d (frame map)", instr->src1);
}

void CodeGenerator8086::emit_scope_exit(IRInstr *instr)
{
    (void)instr;
    if (scope_temp_count > 0 && scope_map_id >= 0) {
        emit_comment("SCOPE_EXIT %d frame refs", scope_temp_count);
        emit_push_imm(scope_temp_count);
        emit_line("mov  ax, seg _SM%d", scope_map_id);
        emit_line("push ax");
        emit_line("mov  ax, offset _SM%d", scope_map_id);
        emit_line("push ax");
        emit_line("push ss");
        emit_line("push bp");
        emit_line("call far ptr pydos_arena_release_frame_");
        emit_line("add  sp, 10");
    }
    if (loop_scope_track_count > 0)
