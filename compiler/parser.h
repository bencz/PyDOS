...
private:
    ...
    ASTNode *parse_while();
    ASTNode *parse_for();
    ASTNode *parse_assignment_target();
    ASTNode *parse_assignment_target_atom();
    ASTNode *parse_try();
    ASTNode *parse_with();
    ASTNode *parse_match();
    ...
    ASTNode *parse_expr();
    ASTNode *parse_expr_or_tuple();
    ASTNode *parse_list_element();
    ASTNode *join_adjacent_strings(ASTNode *node, int sl, int sc);

    /* Enclosing class names for private name mangling. */
    const char *class_stack[16];
    int class_depth;
    const char *mangle_private(const char *name, int len);
