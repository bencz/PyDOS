...
void CodeGenerator386::emit_func(IRFunc *func)
{
    ...
    exception_stub_count = 0;
    exception_stub_capacity = 0;
    for (stub_scan = func->first; stub_scan; stub_scan = stub_scan->next) {
        if (stub_scan->op == IR_CHECK_EXCEPTION && stub_scan->src1 >= 0)
    ...
    if (func != mod->init_func) {
        ...
        emit_line("call pydos_monitoring_py_start_");
        emit_line("add  esp, 4");
...
void CodeGenerator386::emit_builtin_pir_method_registrations()
{
    int i;
    ...
    for (i = 0; i < num_builtin_pir_methods; i++) {
        ...
        emit_line("push %d", binding->runtime_type_tag);
        emit_line("call pydos_builtin_vtable_add_method_");
...
void CodeGenerator386::emit_load_global(IRInstr *instr)
{
    ...
    if (!is_registered_global(gname) && stdlib_reg_ &&
        ...
        emit_comment("LOAD_GLOBAL (PIR function object) '%s' -> t%d",
                     gname, instr->dest);
        emit_line("push offset %s", name_label);
        emit_line("push offset %s", function_label);
        emit_line("call pydos_func_new_");
        emit_line("add  esp, 8");
        emit_line("mov  dword ptr [ebp%+d], eax", slot_offset(dst_slot));
        emit_line("push offset %s", signature_label);
        emit_line("push %d", arg_count);
        emit_line("push dword ptr [ebp%+d]", slot_offset(dst_slot));
        emit_line("call pydos_func_set_signature_");
        emit_line("add  esp, 12");
        return;
    }
    ...
    if (!is_registered_global(gname) && builtin_asm_name(gname)) {
        ...
        if (type_tag >= 0) {
            ...
            emit_comment("LOAD_GLOBAL (type) '%s' -> t%d", gname,
                         instr->dest);
            emit_line("push %d", type_tag);
            emit_line("call pydos_builtin_type_object_");
            emit_line("add  esp, 4");
            emit_line("mov  dword ptr [ebp%+d], eax",
                      slot_offset(dst_slot));
        } else {
            int name_idx = register_mn_string(gname);
            int dst_slot = current_func->num_locals + instr->dest;
            char name_label[32];
            ...
            emit_comment("LOAD_GLOBAL (builtin object) '%s' -> t%d",
                         gname, instr->dest);
            emit_line("push offset %s", name_label);
            emit_line("push offset %s", builtin_asm_name(gname));
            emit_line("call pydos_func_new_builtin_");
            emit_line("add  esp, 8");
            emit_line("mov  dword ptr [ebp%+d], eax",
                      slot_offset(dst_slot));
        }
...
void CodeGenerator386::emit_call(IRInstr *instr)
{
    ...
    if (builtin_name) {
        ...
        if (is_direct) {
            ...
            /* Stack arguments already form argv[0..argc-1] in ascending
             * memory. Route every dynamic call through the object protocol
             * so functions, bound methods, classes and __call__ agree. */
            ...
            emit_line("push %d", argc);
            push_temp(instr->src1);
            emit_line("call pydos_obj_call_");
            emit_line("add  esp, %d", 12 + argc * 4);
            store_eax_to_temp(instr->dest);
            num_call_args = 0;
            return;
        }
...
void CodeGenerator386::emit_call_method(IRInstr *instr)
{
    ...
    if (instr->op == IR_GUARDED_CALL_METHOD) {
        ...
        emit_line("push offset %s", direct_lbl);
    }
    ...
    emit_line(instr->op == IR_GUARDED_CALL_METHOD
              ? "call pydos_obj_call_method_guarded_"
              : "call pydos_obj_call_method_");
    /* name + optional target + argc + argv + argv array */
    emit_line("add  esp, %d",
              (instr->op == IR_GUARDED_CALL_METHOD ? 16 : 12) +
              total_argc * 4);
...
void CodeGenerator386::emit_return(IRInstr *instr)
{
    ...
    if (current_func != mod->init_func) {
        ...
        emit_line("push eax");
        emit_line("push offset %s", flbl);
        emit_line("call pydos_monitoring_py_return_");
        emit_line("add  esp, 8");
        if (instr->src1 >= 0)
            load_temp_to_eax(instr->src1);
        else
...
void CodeGenerator386::emit_build_tuple(IRInstr *instr)
{
    ...
    if (count == 0) {
        ...
        emit_line("call pydos_obj_new_empty_tuple_");
        store_eax_to_temp(instr->dest);
        num_call_args = 0;
        return;
    }
    ...
    push_temp(instr->dest);
    emit_line("call pydos_list_to_tuple_");
    emit_line("add  esp, 4");
    store_eax_to_temp(instr->dest);
...
void CodeGenerator386::emit_for_iter(IRInstr *instr)
{
    ...
    /* NULL without a pending exception is normal exhaustion.  NULL with a
     * pending exception propagates to the active landing pad. */
    store_eax_to_temp(instr->dest);
    emit_line("test eax, eax");
    {
        int has_value = new_label();
        emit_line("jnz  _L%d", has_value);
        emit_line("call pydos_exc_pending_");
        emit_line("test eax, eax");
...
void CodeGenerator386::emit_raise(IRInstr *instr)
{
    ...
    emit_line("add  esp, 4");
    emit_line("push 0");
    emit_line("push offset _MN_%d", name_idx);
    emit_line("call pydos_exc_add_traceback_");
    emit_line("add  esp, 8");
    emit_line("jmp  _L%d", instr->extra);
}
...
void CodeGenerator386::emit_get_exception(IRInstr *instr)
{
    ...
    emit_line("call pydos_exc_fetch_");

    store_eax_to_temp(instr->dest);
}

void CodeGenerator386::emit_check_exception(IRInstr *instr)
{
    int clear_label = new_label();
    int name_idx = register_mn_string(current_func->name);
    ...
    if (instr->src1 >= 0) {
        ...
        load_temp_to_eax(instr->src1);
        emit_line("test eax, eax");
        emit_line("jnz  _LCG%d", clear_label);
        emit_line("jmp  _LCG%d", stub_label);
        emit_line("_LCG%d:", clear_label);
        return;
    }
    require_extern("pydos_exc_pending_");
    emit_line("call pydos_exc_pending_");
    emit_line("test eax, eax");
    emit_line("jz   _LCG%d", clear_label);
    emit_line("push 0");
    emit_line("push offset _MN_%d", name_idx);
    emit_line("call pydos_exc_add_traceback_");
    emit_line("add  esp, 8");
...
int CodeGenerator386::get_exception_stub(int handler_label, int line)
{
    int i;
...
void CodeGenerator386::emit_exception_stubs(IRFunc *func)
{
    int i;
    int name_idx;
    ...
    for (i = 0; i < exception_stub_count; i++) {
        ...
        emit_line("push %d", exception_stub_lines[i]);
        emit_line("push offset _MN_%d", name_idx);
        emit_line("call pydos_exc_require_traceback_");
        emit_line("add  esp, 8");
        emit_line("jmp  _L%d", exception_stub_handlers[i]);
    }
}

void CodeGenerator386::emit_clear_exception(IRInstr *instr)
{
    (void)instr;
    emit_comment("CLEAR_EXCEPTION");
    require_extern("pydos_exc_clear_");
    emit_line("call pydos_exc_clear_");
}

void CodeGenerator386::emit_panic_exception(IRInstr *instr)
{
    (void)instr;
    emit_comment("PANIC_EXCEPTION");
    require_extern("pydos_exc_panic_current_");
    emit_line("call pydos_exc_panic_current_");
}
...
void CodeGenerator386::emit_alloc_obj(IRInstr *instr)
{
    ...
    emit_line("push %d", type_tag);
    emit_line("call pydos_obj_alloc_type_");
    emit_line("add  esp, 4");
...
void CodeGenerator386::emit_init_vtable(IRInstr *instr)
{
    ...
    /* Reserve exactly the number of declared methods. */
    require_extern("pydos_vtable_create_sized_");
    emit_line("push %d", vti->num_methods);
    emit_line("call pydos_vtable_create_sized_");
    emit_line("add  esp, 4");
    ...
    for (mi = 0; mi < vti->num_methods; mi++) {
        ...
        emit_line("push %d", vti->methods[mi].arg_count);
        ...
        emit_line("call pydos_vtable_add_method_sig_");
        emit_line("add  esp, 16");
    }
    ...
    /* Materialize the Python class object and bind it to the class global. */
    {
        ...
        emit_line("push dword ptr [%s]", vt_label);
        emit_line("push offset %s", cn_lbl);
        emit_line("call pydos_class_new_");
        emit_line("add  esp, 8");
        emit_line("mov  dword ptr [%s], eax", class_global);

        if ((!vti->base_class_name || vti->base_class_name[0] == '\0') &&
            vti->num_extra_bases == 0) {
            emit_line("push dword ptr [%s]", class_global);
            emit_line("call pydos_class_add_object_base_");
            emit_line("add  esp, 4");
        }
        ...
        for (bi = 0; bi < 1 + vti->num_extra_bases; bi++) {
            ...
            if (runtime_tag >= 0) {
                require_extern("pydos_builtin_type_object_");
                emit_line("push %d", runtime_tag);
                emit_line("call pydos_builtin_type_object_");
                emit_line("add  esp, 4");
                emit_line("push eax");
                emit_line("push dword ptr [%s]", class_global);
                emit_line("call pydos_class_add_base_");
                emit_line("add  esp, 8");
                continue;
            }
            base_global = global_asm_name(base);
            emit_line("push dword ptr [%s]", base_global);
            emit_line("push dword ptr [%s]", class_global);
            emit_line("call pydos_class_add_base_");
            emit_line("add  esp, 8");
        }
...
void CodeGenerator386::emit_set_vtable(IRInstr *instr)
{
...

    {
        ...
        emit_line("push dword ptr [%s]", class_global);
        push_temp(instr->src1);
        emit_line("call pydos_obj_set_class_");
        emit_line("add  esp, 8");
    }
}
...
void CodeGenerator386::emit_make_function(IRInstr *instr)
{
    ...
    /* Preserve the generated fixed ABI arity on first-class functions so
     * dynamic assignment/binding still rejects missing or excess args. */
    {
        IRFunc *defined;
        int arg_count = -1;
        for (defined = mod->functions; defined; defined = defined->next) {
        ...
        if (arg_count >= 0) {
            ...
            load_temp_to_eax(instr->dest);
            emit_line("push eax");
            emit_line("call pydos_func_set_arg_count_");
            emit_line("add  esp, 8");
        }
        load_temp_to_eax(instr->dest);
    }
