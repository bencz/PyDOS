...
Token Lexer::next_token() {
    ...
    for (;;) {
        ...
        if (c == '\n') {
            if (fstring_depth > 0 &&
                fstring_brace_depth[fstring_depth - 1] > 0) {
                /* PEP 701 permits physical newlines and comments inside an
                 * f-string replacement field without surrounding parens. */
                continue;
            }
            if (paren_depth > 0) {
