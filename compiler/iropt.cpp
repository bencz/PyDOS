...
int IROptimizer::is_pure(IROp op)
{
    ...
    case IR_CONST_NONE:
    case IR_CONST_BOOL:
    /* Identity tests do not invoke Python protocols. */
    case IR_IS:
    case IR_IS_NOT:
...
{
    (void)func;
    /* Boxed Python arithmetic can dispatch to user code.  Rewriting x * 0,
     * x * 1 or x * 2 changes observable calls and result types for sequences
     * and overloaded objects.  Strength reduction belongs on typed,
     * unboxed IR after an exact-type proof exists. */
}
/* ================================================================ */
