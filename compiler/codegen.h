...
#define CODEGEN_MAX_VTABLES     128

/* Maximum number of method-name string constants */
#define CODEGEN_MAX_MN_STRINGS  128

/* Maximum number of Python-backed builtin methods linked into one module. */
#define CODEGEN_MAX_BUILTIN_PIR_METHODS 64
...
protected:
    ...
    struct GlobalVar {
        ...
        char asm_name[256];     /* case-safe assembly label: _G_name */
    };
    GlobalVar *globals;
    int globals_cap;
    int num_globals;
    void register_global(const char *name);
    int is_registered_global(const char *name) const;
    ...
    /* --------------------------------------------------------------- */
    struct VTableGlobal {
        char label[256];          /* case-safe _VT_ClassName */
    };
    ...
    struct BuiltinPIRMethodBinding {
        int runtime_type_tag;
        const char *method_name;
        const char *func_name;
        int mn_string_index;
    };
    BuiltinPIRMethodBinding builtin_pir_methods[CODEGEN_MAX_BUILTIN_PIR_METHODS];
    int num_builtin_pir_methods;
    ...
    /* Target hook used to invalidate ABI state after an emitted call. */
    virtual void note_call_emitted();
    ...
    void mangle_identifier(char *buf, int bufsize, const char *name);
    void func_label(char *buf, int bufsize, const char *funcname);
    void vtable_label(char *buf, int bufsize, const char *classname);
    ...
    void collect_builtin_pir_methods();
    ...
    /* Register linked Python-backed methods in primitive-type vtables. */
    virtual void emit_builtin_pir_method_registrations() = 0;
    ...
    virtual void emit_check_exception(IRInstr *instr) = 0;
    virtual void emit_clear_exception(IRInstr *instr) = 0;
    virtual void emit_panic_exception(IRInstr *instr) = 0;
