...
SemanticAnalyzer::SemanticAnalyzer()
{
    ...
    next_slot = 0;
    num_expr_types = 0;
    num_generic_aliases = 0;
    alias_expansion_depth = 0;
    memset(alias_expansion_stack, 0, sizeof(alias_expansion_stack));
    memset(generic_aliases, 0, sizeof(generic_aliases));
...
int SemanticAnalyzer::analyze(ASTNode *module)
{
    ...
    /* analyze() is a complete analysis pass, not an incremental fragment
     * operation.  Monomorphization used to call it repeatedly and each call
     * left another global scope below the new one.  Besides leaking scopes,
     * this allowed ordinary local assignments to resolve unrelated symbols
...
int SemanticAnalyzer::analyze_fragment(ASTNode *node)
{
    /* Analyze generated definitions in the already established module scope.
     * This intentionally preserves both the original symbols and expression
     * type annotations.  It is used by monomorphization after specialize(). */
    if (!node || !current_scope) return error_count;
    analyze_stmt(node);
    return error_count;
}
...
Symbol *SemanticAnalyzer::resolve_current(const char *name)
{
    Symbol *sym;

    if (!current_scope || !name) return 0;
    for (sym = current_scope->symbols; sym; sym = sym->next) {
...
static TypeInfo *type_kind_to_info(int kind)
{
    ...
    case TY_TUPLE:  return type_new(TY_TUPLE, "tuple");
    case TY_SET:    return type_new_set(type_any);
    case TY_BYTES:  return type_bytes;
    case TY_RANGE:  return type_range;
    case TY_FROZENSET: return type_frozenset;
    case TY_COMPLEX:   return type_complex;
    case TY_BYTEARRAY: return type_bytearray;
...
void SemanticAnalyzer::register_builtins()
{
    ...
    declare("NotImplemented", type_any, SYM_BUILTIN);
    declare("locals", type_any, SYM_BUILTIN);
    declare("format", type_any, SYM_BUILTIN);
    declare("memoryview", type_any, SYM_BUILTIN);
}
                ...
                for (s = current_scope->parent; s; s = s->parent) {
                    ...
                    for (outer_sym = s->symbols; outer_sym; outer_sym = outer_sym->next) {
                        if (strcmp(outer_sym->name, vname) == 0) {
                            ...
                            /* Function scopes between the declaration and the
                             * owner pass the cell down through their own
                             * closure, so they must carry the name too. */
                            {
                                Scope *between;
                                for (between = current_scope->parent;
                                     between && between != s;
                                     between = between->parent) {
                                    if (!between->is_function) continue;
                                    already = 0;
                                    for (j = 0; j < between->num_free_vars; j++) {
                                        if (strcmp(between->free_vars[j],
                                                   vname) == 0) already = 1;
                                    }
                                    if (!already && between->num_free_vars < 32)
                                        between->free_vars[between->num_free_vars++] =
                                            str_dup(vname);
                                }
                            }
...
/* Test for a plain "@name" decorator on a function definition. */
static int func_has_decorator(ASTNode *node, const char *name)
{
    ASTNode *decorator;
    if (!node) return 0;
    for (decorator = node->data.func_def.decorators; decorator;
         decorator = decorator->next) {
        if (decorator->kind == AST_NAME && decorator->data.name.id &&
            strcmp(decorator->data.name.id, name) == 0) return 1;
    }
    return 0;
}
...
void SemanticAnalyzer::analyze_funcdef(ASTNode *node)
{
    ...
    if (node->data.func_def.return_type) {
        ...
        /* Python does not infer an omitted annotation as -> None.  Keeping
         * it dynamic also permits CPython-valid functions such as an
         * __init__ that returns a value and consequently fails at call time. */
        ret_type = type_any;
    }
    ...
    for (p = node->data.func_def.params; p; p = p->next) {
        ...
        if (p->annotation) {
            pt = resolve_type(p->annotation);
                   ...
                   !func_has_decorator(node, "staticmethod")) {
    ...
    {
        Symbol *defined = declare(node->data.func_def.name, func_type, SYM_FUNC);
        if (defined) {
            defined->definition = node;
            if (node->data.func_def.decorators)
                defined->is_decorated = 1;
        }
    }
...
void SemanticAnalyzer::analyze_classdef(ASTNode *node)
{
        ...
        if (ci->bases) {
            ...
            for (base_node = node->data.class_def.bases; base_node; base_node = base_node->next) {
                ...
                if (base_sym && base_sym->type && base_sym->type->class_info) {
                    ...
                    idx++;
                } else if (base_node->kind == AST_NAME &&
                           base_node->data.name.id &&
                           strcmp(base_node->data.name.id, "type") == 0) {
                    /* `type` is a runtime class object rather than a
                     * user ClassInfo; code generation attaches it directly. */
                } else if (base_node->kind == AST_NAME &&
                           base_node->data.name.id &&
                           (strcmp(base_node->data.name.id, "TypedDict") == 0 ||
                            strcmp(base_node->data.name.id, "Protocol") == 0)) {
                    /* Static typing bases do not contribute runtime layout. */
    ...
    {
        Symbol *defined = declare(node->data.class_def.name,
                                  class_type, SYM_CLASS);
        if (defined && node->data.class_def.decorators)
            defined->is_decorated = 1;
    }

    if (node->data.class_def.metaclass)
        analyze_expr(node->data.class_def.metaclass);
    for (base_node = node->data.class_def.keywords;
         base_node; base_node = base_node->next)
        analyze_expr(base_node);
        ...
        if (body->kind == AST_FUNC_DEF && body->data.func_def.name) {
            ...
            if (method) {
                ...
                if (mem) {
                    ...
                    mem->kind = SYM_FUNC;
                    mem->is_decorated = method->is_decorated;
                    mem->definition = body;
                    mem->next = ci->members;
                    ci->members = mem;
                    ci->vtable_size++;
...
void SemanticAnalyzer::analyze_assign(ASTNode *node)
{
    ...
    for (tgt = node->data.assign.targets; tgt; tgt = tgt->next) {
        if (tgt->kind == AST_NAME && tgt->data.name.id) {
            /* Python determines assignment scope lexically: a plain name
             * assigned in a function is local unless a global/nonlocal
             * declaration created a marker in this scope.  Looking through
             * parent scopes here made e.g. a local `result` overwrite an
             * unrelated module-level `result`. */
            existing = resolve_current(tgt->data.name.id);
            if (existing) {
            ...
            for (telem = tgt->data.collection.elts; telem; telem = telem->next) {
                if (telem->kind == AST_NAME && telem->data.name.id) {
                    existing = resolve_current(telem->data.name.id);
                    if (!existing) {
                        declare(telem->data.name.id, type_any, SYM_VAR);
                    }
                }
...
void SemanticAnalyzer::analyze_return(ASTNode *node)
{
    ...
    if (node->data.ret.value) analyze_expr(node->data.ret.value);

    /* Python annotations guide specialization but do not reject a function
     * whose inferred return type differs from its annotation. */
}
    ...
    if (node->kind == AST_IMPORT) {
        ...
        for (imp = node->data.import_from.names; imp; imp = imp->next) {
            if (imp->kind == AST_IMPORT_NAME) {
                ...
                /* Source-linked modules are analyzed before the import
                 * statement, so their real definition is already present.
                 * Do not replace it with the scanner's Any placeholder. */
                if (!resolve_current(name))
...
ASTNode *SemanticAnalyzer::find_generic_alias(const char *name)
{
    int i;
    if (!name) return 0;
    for (i = 0; i < num_generic_aliases; i++) {
        if (strcmp(generic_aliases[i].name, name) == 0)
            return generic_aliases[i].node;
    }
    return 0;
}
...
void SemanticAnalyzer::analyze_type_alias(ASTNode *node)
{
    ...
    /* A parameterised alias is expanded per use, so keep its AST. */
    if (node->data.type_alias.num_type_params > 0 &&
        num_generic_aliases < 64) {
        generic_aliases[num_generic_aliases].name = node->data.type_alias.name;
        generic_aliases[num_generic_aliases].node = node;
        num_generic_aliases++;
    }
...
/* A comprehension target may unpack: [key for key, value in pairs]. */
void SemanticAnalyzer::declare_comprehension_target(ASTNode *target)
{
    ASTNode *element;
    if (!target) return;
    if (target->kind == AST_NAME && target->data.name.id) {
        declare(target->data.name.id, type_any, SYM_VAR);
        return;
    }
    if (target->kind == AST_TUPLE_EXPR || target->kind == AST_LIST_EXPR) {
        for (element = target->data.collection.elts; element;
             element = element->next) {
            declare_comprehension_target(element);
        }
        return;
    }
    if (target->kind == AST_STARRED) {
        declare_comprehension_target(target->data.starred.value);
    }
}
    ...
    case AST_GENEXPR: {
        ...
        while (gen) {
            if (gen->kind == AST_COMP_GENERATOR) {
                ...
                declare_comprehension_target(gen->data.comp_gen.target);
    ...
    case AST_DICTCOMP: {
        ...
        while (dgen) {
            if (dgen->kind == AST_COMP_GENERATOR) {
                ...
                declare_comprehension_target(dgen->data.comp_gen.target);
...
TypeInfo *SemanticAnalyzer::analyze_binop(ASTNode *node)
{
    ...
    case OP_ADD:
        ...
        if (left_type->kind == TY_BYTEARRAY &&
            right_type->kind == TY_BYTEARRAY) return type_bytearray;
    ...
    case OP_MUL:
        ...
        if (left_type->kind == TY_TUPLE && type_is_numeric(right_type)) return left_type;
        if (type_is_numeric(left_type) && right_type->kind == TY_TUPLE) return right_type;
        if (left_type->kind == TY_BYTES && type_is_numeric(right_type)) return type_bytes;
        if (type_is_numeric(left_type) && right_type->kind == TY_BYTES) return type_bytes;
        if (left_type->kind == TY_BYTEARRAY && type_is_numeric(right_type))
            return type_bytearray;
        if (type_is_numeric(left_type) && right_type->kind == TY_BYTEARRAY)
            return type_bytearray;
...
TypeInfo *SemanticAnalyzer::analyze_call(ASTNode *node)
{
    ...
    /* A decorator can replace either a function or a class with any
     * callable object.  Retaining the pre-decoration return type here would
     * enable invalid direct-call/devirtualization optimizations. */
    if (node->data.call.func && node->data.call.func->kind == AST_NAME &&
        node->data.call.func->data.name.id) {
        Symbol *called = resolve(node->data.call.func->data.name.id);
        if (called && called->is_decorated) return type_any;
    }
        ...
        while (ci) {
            for (mem = ci->members; mem; mem = mem->next) {
                if (strcmp(mem->name, attr) == 0) {
                    if (mem->is_decorated) return type_any;
                    return mem->type;
                }
            }
            ci = ci->base;
...
TypeInfo *SemanticAnalyzer::analyze_subscript(ASTNode *node)
{
    ...
    case TY_LIST:
        if (node->data.subscript.index &&
            node->data.subscript.index->kind == AST_SLICE) return obj_type;
    ...
    case TY_RANGE:
        if (node->data.subscript.index &&
            node->data.subscript.index->kind == AST_SLICE) return type_range;
        return type_int;
    default:
        if (node->data.subscript.object &&
            node->data.subscript.object->kind == AST_NAME) {
            const char *name = node->data.subscript.object->data.name.id;
            if (name && (strcmp(name, "tuple") == 0 ||
                         strcmp(name, "list") == 0 ||
                         strcmp(name, "dict") == 0 ||
                         strcmp(name, "set") == 0 ||
                         strcmp(name, "type") == 0))
...
TypeInfo *SemanticAnalyzer::analyze_name(ASTNode *node)
{
    ...
    /* A read from an enclosing function is an implicit closure capture;
     * Python does not require an explicit nonlocal declaration unless the
     * inner function assigns to that name. */
    if (current_scope && current_scope->is_function &&
        scope && scope != current_scope && scope->is_function &&
        !sym->is_global) {
        int i;
        int present = 0;
        sym->is_captured = 1;
        for (i = 0; i < scope->num_cell_vars; i++) {
            if (strcmp(scope->cell_vars[i], id) == 0) present = 1;
        }
        if (!present && scope->num_cell_vars < 32)
            scope->cell_vars[scope->num_cell_vars++] = str_dup(id);

        present = 0;
        for (i = 0; i < current_scope->num_free_vars; i++) {
            if (strcmp(current_scope->free_vars[i], id) == 0) present = 1;
        }
        if (!present && current_scope->num_free_vars < 32)
            current_scope->free_vars[current_scope->num_free_vars++] =
                str_dup(id);

        /* Every function scope between the use and the owner must carry the
         * name too: the closure of an intermediate function is what hands the
         * cell down to the innermost one. */
        {
            Scope *between;
            for (between = current_scope->parent;
                 between && between != scope;
                 between = between->parent) {
                if (!between->is_function) continue;
                present = 0;
                for (i = 0; i < between->num_free_vars; i++) {
                    if (strcmp(between->free_vars[i], id) == 0) present = 1;
                }
                if (!present && between->num_free_vars < 32)
                    between->free_vars[between->num_free_vars++] = str_dup(id);
            }
        }
    }
    return sym->type;
...
TypeInfo *SemanticAnalyzer::analyze_lambda(ASTNode *node)
{
    ...
    if (current_scope->num_free_vars > 0) {
        int fv;
        node->data.lambda.free_var_names =
            name_array_alloc(current_scope->num_free_vars);
        node->data.lambda.num_free_vars = current_scope->num_free_vars;
        for (fv = 0; fv < current_scope->num_free_vars; fv++)
            node->data.lambda.free_var_names[fv] =
                current_scope->free_vars[fv];
    }
...
TypeInfo *SemanticAnalyzer::resolve_type(ASTNode *type_node)
{
    ...
    case AST_TYPE_NAME: {
        ...
        /* In annotation position "type" means a class object, not the
         * signature of the builtin funcs.py declares under that name. */
        if (strcmp(name, "type") == 0)   return type_any;
    ...
    case AST_TYPE_GENERIC: {
        ...
        /* Parameterised alias: bind the parameters and resolve the RHS. */
        {
            ASTNode *alias = find_generic_alias(base_name);
            if (alias && alias_expansion_depth < 16) {
                TypeInfo *expanded;
                TypeInfo *bound = resolved_args;
                int p;
                int active = 0;

                /* A recursive PEP 695 alias is a recursive type graph, not a
                 * request to clone its RHS indefinitely.  The static checker
                 * uses Any at the back-edge while the runtime alias object
                 * retains the recursive expression for reflection. */
                for (p = 0; p < alias_expansion_depth; p++) {
                    if (alias_expansion_stack[p] == alias) {
                        active = 1;
                        break;
                    }
                }
                if (active) return type_any;

                alias_expansion_stack[alias_expansion_depth] = alias;
                alias_expansion_depth++;
                push_scope(0, 0);
                for (p = 0; p < alias->data.type_alias.num_type_params; p++) {
                    declare(alias->data.type_alias.type_param_names[p],
                            bound ? bound : type_any, SYM_BUILTIN);
                    if (bound) bound = bound->next;
                }
                expanded = resolve_type(alias->data.type_alias.value);
                pop_scope();
                alias_expansion_depth--;
                alias_expansion_stack[alias_expansion_depth] = 0;
                return expanded ? expanded : type_any;
            }
        }
