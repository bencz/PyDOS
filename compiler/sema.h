...
struct Symbol {
    ...
    int is_decorated;   /* runtime binding may differ from declared object */
    ASTNode *definition;/* defining function/class AST, when available */
...
public:
    ...
    int analyze_fragment(ASTNode *node);
...
private:
    ...
    /* Generic type aliases keep their AST so "P[int]" can re-resolve the
     * right-hand side with the parameters bound to the arguments. */
    struct GenericAlias {
        const char *name;
        ASTNode *node;
    };
    GenericAlias generic_aliases[64];
    int num_generic_aliases;
    int alias_expansion_depth;
    ASTNode *alias_expansion_stack[16];
    ASTNode *find_generic_alias(const char *name);
    void declare_comprehension_target(ASTNode *target);

    /* Scope management */
    void push_scope(int is_function, int is_class);
    void pop_scope();
    Symbol *declare(const char *name, TypeInfo *type, SymbolKind kind);
    Symbol *resolve_current(const char *name);
