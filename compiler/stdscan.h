...
#define STDLIB_MAX_PIR_FUNCS  128
...
struct BuiltinFuncEntry {
    ...
    /* C-backed: assembler symbol.  PIR-backed: compact Python parameter
     * signature used to bind keyword arguments and materialize defaults. */
...
struct BuiltinMethodEntry {
    ...
    /* C-backed: assembler symbol. PIR-backed: compact parameter signature
     * excluding self, used for keyword/default binding. */
    char asm_name[STDLIB_ASM_LEN];
...
public:
     ...
     * Returns NULL if not found or if Python-backed. */
    const char *find_builtin_asm_name(const char *name) const;

    /* Return the parameter signature stored for a Python/PIR builtin. */
    const char *find_pir_signature(const char *name) const;
     ...
     * If found, writes the PIR function name (e.g., "list__count") to
