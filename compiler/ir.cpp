...
static const char *irop_names[] = {
    ...
    "CHECK_EXCEPTION", "CLEAR_EXCEPTION", "PANIC_EXCEPTION",
    "YIELD", "YIELD_FROM",
    "MAKE_FUNCTION",
...
void ir_dump_func(IRFunc *func, FILE *out)
{
    ...
    while (ins) {
        ...
        case IR_GUARDED_CALL_METHOD:
            dump_operand(ins->dest, out);
            fprintf(out, ins->op == IR_GUARDED_CALL_METHOD
                         ? " = GUARDED_CALL_METHOD " : " = CALL_METHOD ");
            dump_operand(ins->src1, out);
            fprintf(out, ".[%d] argc=%d", ins->src2, ins->extra);
            if (ins->guard_target)
                fprintf(out, " guard=@%s", ins->guard_target);
            break;
        ...
        case IR_CHECK_EXCEPTION:
            fprintf(out, "    CHECK_EXCEPTION handler=L%d", ins->extra);
            break;

        case IR_CLEAR_EXCEPTION:
            fprintf(out, "    CLEAR_EXCEPTION");
            break;

        case IR_PANIC_EXCEPTION:
            fprintf(out, "    PANIC_EXCEPTION");
            break;
