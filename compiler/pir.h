...
enum PIROp {
    ...
    PIR_CALL,               /* call str_val or explicit callable operands[0] */
    PIR_CALL_METHOD,        /* result:pyobj = operands[0].str_val(int_val args) */
    PIR_GUARDED_CALL_METHOD,/* CALL_METHOD with guard_target direct fast path */
    ...
    PIR_CHECK_EXCEPTION,    /* branch to handler_block if exception pending */
    PIR_CLEAR_EXCEPTION,    /* consume current pending exception */
    PIR_PANIC_EXCEPTION,    /* report current exception as unhandled */
...
struct PIRInst {
    ...
    const char *guard_target;   /* direct target for guarded method calls */
...
struct PIRFunction {
    ...
    /* Monomorphization provenance.  Functions with matching non-null values
     * may share one implementation when their optimized PIR is identical. */
    const char *generic_origin;
    const char *generic_method_name;
...
struct PIRVTableMethod {
    ...
    int arg_count;              /* fixed generated ABI argc, including self */
};

struct PIRVTableInfo {
    const char *class_name;
    const char *display_name;       /* Python-visible name, 0 if same */
