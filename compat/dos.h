/*
 * dos.h - flat-host compatibility stub for Open Watcom dos.h
...
#define PYDOS_COMPAT_UNUSED __attribute__((unused))
#else
#define PYDOS_COMPAT_UNUSED
#endif

/* PYDOS_API calling convention - no-op on native flat-memory test hosts */
#ifndef PYDOS_API
#define PYDOS_API
#endif
...
static PYDOS_COMPAT_UNUSED int int86(int intno, union REGS *in, union REGS *out)
{
    (void)intno; (void)in;
    memset(out, 0, sizeof(*out));
    return 0;
}
static PYDOS_COMPAT_UNUSED int int86x(int intno, union REGS *in, union REGS *out, struct SREGS *sr)
{
    (void)intno; (void)in; (void)sr;
    memset(out, 0, sizeof(*out));
    return 0;
}
static PYDOS_COMPAT_UNUSED void segread(struct SREGS *sr)
{
...
static PYDOS_COMPAT_UNUSED char *ltoa(long value, char *buf, int radix)
{
    (void)radix;
    sprintf(buf, "%ld", value);
    return buf;
}
static PYDOS_COMPAT_UNUSED char *ultoa(unsigned long value, char *buf, int radix)
{
    (void)radix;
    sprintf(buf, "%lu", value);
    return buf;
}
static PYDOS_COMPAT_UNUSED char *itoa(int value, char *buf, int radix)
{
    (void)radix;
    sprintf(buf, "%d", value);
    return buf;
